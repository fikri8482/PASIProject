        ...
        Try
            Using cn As New SqlConnection(pConstr)
                ...
                If i = 0 Then
                    ...
                    sql = "SELECT ISNULL(a.POMOQ,b.MOQ) MOQ, ISNULL(a.POQtyBox,b.QtyBox) Qty FROM dbo.PO_Detail_Export a left join MS_PartMapping b on a.PartNo = b.PartNo and a.AffiliateID = b.AffiliateID and a.SupplierID = b.SupplierID " & vbCrLf & _
                     "WHERE PONo ='" + Trim(pMaster.PONo) + "' AND a.PartNo = '" + Trim(pMaster.PartNo) + "' AND a.SupplierID = '" + Trim(pMaster.SupplierID) + "' AND a.AffiliateID = '" + Trim(pMaster.AffiliateID) + "' "
                    ...
                    If dt.Tables(0).Rows.Count > 0 Then
                        pomoq = dt.Tables(0).Rows(0)("MOQ")
                        poqty = dt.Tables(0).Rows(0)("Qty")
                    End If


                    sql = " INSERT INTO dbo.DOSUpplier_Detail_Export " & vbCrLf & _
                            " (SuratJalanNo, SupplierID, AffiliateID, PONo, PartNo, OrderNo, SeqNo, DOQty, POMOQ, POQtyBox) " & vbCrLf & _
                            " VALUES  ( '" & Trim(pMaster.SuratJalanNo) & "' , " & vbCrLf & _
                            "           '" & Trim(pMaster.SupplierID) & "' , " & vbCrLf & _
                            "           '" & Trim(pMaster.AffiliateID) & "' ,  " & vbCrLf & _
                            "           '" & Trim(pMaster.PONo) & "' ,  " & vbCrLf & _
