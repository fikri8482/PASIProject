...
#Region "CONTROL EVENTS"
    'Private Sub KanbanList_Init(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Init
    ...
    '        Call up_GridLoad(cbosupplier.Text, dt1.Value, dt2.Value)
    '    Else
    '        Clear()
        ...
        Try
            ...
            If (Not IsPostBack) AndAlso (Not IsCallback) Then
                If Not IsNothing(Request.QueryString("prm")) Then
                    ...
                    Dim pcbolocation As String = Split(param, "|")(4)
                    Dim pLocationname As String = Split(param, "|")(5)
                    Dim pKanbanno As String = Split(param, "|")(5)

                    Call up_fillcombo()
                    Call up_FillComboKanban(pdt1, pdt2, pcbosupplier, pcbolocation)
                    ...
                    Call up_GridLoad(cbosupplier.Text, dt1.Value, dt2.Value, cbokanbanno)
                Else
                    Clear()
                    ...
                    If Session("KCR-Load") <> "YES" And Not IsNothing(Session("KCR-Load")) Then
                        ...
                        cbolocation.Text = Session("KCR-cboloc")
                        txtlocation.Text = Session("KCR-txtloc")
                        Call up_GridLoad(cbosupplier.Text, Session("KCR-dt1"), Session("KCR-dt2"), Session("kanbanno"))
                    Else
                        Call up_fillcombo()
                        lblerrmessage.Text = ""
    ...
    Protected Sub grid_BatchUpdate(ByVal sender As Object, ByVal e As DevExpress.Web.Data.ASPxDataBatchUpdateEventArgs) Handles grid.BatchUpdate
        ...
        'ls_KanbanDate = ""  
        'For i = 0 To e.UpdateValues.Count - 1
        '    ls_Active = (e.UpdateValues(i).NewValues("cols").ToString())
        ...
        'If ls_KanbanDate <> "" And ls_Supplier <> "" Then
    ...
    Protected Sub grid_CustomCallback(sender As Object, e As DevExpress.Web.ASPxGridView.ASPxGridViewCustomCallbackEventArgs) Handles grid.CustomCallback
        Try
            ...
            Select Case pAction
                Case "gridload"
                    Dim pSupplier As String = Split(e.Parameters, "|")(1)
                    Dim pDateFrom As Date = Split(e.Parameters, "|")(2)
                    Dim pDateTo As Date = Split(e.Parameters, "|")(3)
                    Dim pKanbanno As String = Split(e.Parameters, "|")(4)


                    Call up_GridLoad(pSupplier, pDateFrom, pDateTo, pKanbanno)
                ...
                Case "PrintCard"
                    If Session("KCR-KanbanDate") <> "" Then
                        If Session("KCR-Approve") = True Then
                            ...
                            DevExpress.Web.ASPxClasses.ASPxWebControl.RedirectOnCallback("~/Kanban/ViewReport.aspx")
                        Else
                            Call clsMsg.DisplayMessage(lblerrmessage, "7011", clsMessage.MsgType.ErrorMessage)
                            grid.JSProperties("cpMessage") = lblerrmessage.Text
                        End If
...
#Region "PROCEDURE"
    ...
    Private Sub up_FillComboKanban(ByVal pdate1 As Date, ByVal pdate2 As Date, ByVal psupplier As String, ByVal plocation As String)
        ...
        ls_sql = " SELECT distinct kanbanNo = '" & clsGlobal.gs_All & "' from Kanban_Master union all SELECT distinct KanbanNo = RTRIM(KanbanNo) " & vbCrLf & _
                 " FROM Kanban_Master where AffiliateID = '" & Session("AffiliateID") & "' " & vbCrLf & _
    ...
    Private Sub up_fillcombo()
        ...
        'Delivery Location
        ls_sql = "SELECT distinct [Delivery Location Code] = '" & clsGlobal.gs_All & "', [Delivery Location Name] ='" & clsGlobal.gs_All & "' from MS_supplier union all SELECT [Delivery Location Code] = RTRIM(DeliveryLocationCode) ,[Delivery Location Name] = RTRIM(DeliveryLocationName) FROM MS_DeliveryPlace where AffiliateID = '" & Session("AffiliateID") & "' " & vbCrLf
        Using sqlConn As New SqlConnection(clsGlobal.ConnectionString)
            sqlConn.Open()
    ...
    Private Sub Clear()
        ...
        txtlocation.Text = clsGlobal.gs_All
        cbokanbanno.Text = clsGlobal.gs_All
    End Sub

    Private Sub up_GridLoad(ByVal psupplier, ByVal pdatefrom, ByVal pdateto, ByVal pkanbanno)
        Dim ls_SQL As String = ""
        ...
        Using sqlConn As New SqlConnection(clsGlobal.ConnectionString)
            ...
            ls_SQL = "  SELECT " & vbCrLf & _
                     ...
                     "  colcreateddate = CONVERT(CHAR(20),CONVERT(DATETIME,KM.EntryDate),113), --(select TOP 1 CONVERT(CHAR(20),CONVERT(DATETIME,EntryDate),113)  " & vbCrLf & _
                     ...
                     "  coldetailurl = 'KanbanCreate.aspx?prm='+CONVERT(CHAR(12),CONVERT(DATETIME,KanbanDate),106) " & vbCrLf
            ...
            ls_SQL = ls_SQL + "  FROM dbo.Kanban_Master KM  " & vbCrLf & _
                              ...
                              "  LEFT JOIN MS_DeliveryPlace MDP ON KM.DeliveryLocationCode = MDP.DeliveryLocationCode "
            ...
            ls_SQL = ls_SQL + "  LEFT JOIN dbo.SC_UserSetup MSAE ON MSAE.UserID = KM.EntryUser AND UserCls = 0 " & vbCrLf & _
                              ...
                              "  WHERE KanbanDate between '" & Format(pdatefrom, "yyyy-MM-dd") & "' and '" & Format(pdateto, "yyyy-MM-dd") & "' and KM.AffiliateID = '" & Session("AffiliateID") & "' " & vbCrLf

            If (cbosupplier.Text <> clsGlobal.gs_All And cbosupplier.Text <> "") Or (psupplier <> clsGlobal.gs_All And psupplier <> "") Then
            ...
            If (cbokanbanno.Text <> clsGlobal.gs_All And cbokanbanno.Text <> "") Then
                ls_SQL = ls_SQL + " and KM.kanbanno = '" & Trim(cbokanbanno.Text) & "'"
            End If
...
#End Region
    ...
    Private Sub cbokanbanno_Callback(ByVal sender As Object, ByVal e As DevExpress.Web.ASPxClasses.CallbackEventArgsBase) Handles cbokanbanno.Callback
        ...
        Call up_FillComboKanban(ls_value1, ls_value2, ls_supplier, ls_location)
