...
#Region "FORM EVENTS"
    ...
    Private Sub btnSubMenu_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnSubMenu.Click
        Response.Redirect("~/Master/ETDPASIMaster.aspx")
    End Sub
...
#Region "PROCEDURE"
    ...
    Private Sub bindData()
        ...
        Using sqlConn As New SqlConnection(clsGlobal.ConnectionString)
            ...
            ls_SQL = " select  " & vbCrLf & _
                  " 	row_number() over (order by AffiliateID, ETAAffiliate asc) as NoUrut, " & vbCrLf & _
                  " 	AffiliateID, ETAAffiliate, ETDPASI, ErrorCls "

            ls_SQL = ls_SQL + " from UploadETDPASI order by AffiliateID, ETAAffiliate "
    ...
    Private Sub up_GridLoadWhenEventChange()
        ...
        Using sqlConn As New SqlConnection(clsGlobal.ConnectionString)
            ...
            ls_SQL = " select top 0 '' NoUrut, '' AffiliateID, '' ETAAffiliate, '' ETDPASI , '' ErrorCls"
            ...
            If Uploader.HasFile Then
                ...
                Using sqlConn As New SqlConnection(clsGlobal.ConnectionString)
                    ...
                    Try
                        ...
                        MyCommand.CommandText = ("SELECT * FROM [" & pTableCode & "A3:C65536]")
                        ...
                        If dtDetail.Rows.Count > 0 Then
                            For i = 0 To dtDetail.Rows.Count - 1
                                If IsDBNull(dtDetail.Rows(i).Item(0)) = False Then
                                    ...
                                    dtUploadDetail.ETDAffiliate = dtDetail.Rows(i).Item(1)
                                    'dtUploadDetail.ETDPASI = dtDetail.Rows(i).Item(2)
                                    If IsDBNull(dtDetail.Rows(i).Item(2)) = False Then
                                        dtUploadDetail.ETDPASI = dtDetail.Rows(i).Item(2)
                                    Else
                                        dtUploadDetail.ETDPASI = "NULL"
                                    End If
                        ...
                        Using sqlTran As SqlTransaction = sqlConn.BeginTransaction("uploadPO")
                            ...
                            For i = 0 To dtUploadDetailList.Count - 1
                                ...
                                If IsDate(PO.ETDAffiliate) = False Then
                                    If ls_error = "" Then
                                ...
                                If PO.ETDPASI <> "NULL" Then
                                    If IsDate(PO.ETDPASI) = False Then
                                        If ls_error = "" Then
                                            ls_error = ls_error & "Invalid format date, please check again"
                                        Else
                                            ls_error = ls_error & "; " & "Invalid format date, please check again"
                                ...
                                ls_sql = " INSERT INTO [dbo].[UploadETDPASI] " & vbCrLf & _
                                          "            ([AffiliateID], [ETAAffiliate], [ETDPASI],[ErrorCls]) " & vbCrLf & _
                                          "      VALUES " & vbCrLf & _
                                          "            ('" & PO.AffiliateID & "'" & vbCrLf & _
                                          "            ,'" & PO.ETDAffiliate & "' " & vbCrLf & _
                                          "            ," & IIf(PO.ETDPASI = "NULL", PO.ETDPASI, "'" & PO.ETDPASI & "'") & "" & vbCrLf & _
                                          "            ,'" & ls_error & "') " & vbCrLf
            ...
            Try
                ...
                For i = 0 To grid.VisibleRowCount - 1
                    If grid.GetRowValues(i, "AffiliateID") <> "" Then                        
                        ls_Sql = " IF NOT EXISTS (select * from MS_ETD_PASI where AffiliateID = '" & grid.GetRowValues(i, "AffiliateID") & "' and ETAAffiliate = '" & grid.GetRowValues(i, "ETAAffiliate") & "')" & vbCrLf & _
                                  " BEGIN" & vbCrLf & _
                                  "      INSERT INTO [dbo].[MS_ETD_PASI] " & vbCrLf & _
                                  ...
                                  "            ," & IIf(IsDBNull(grid.GetRowValues(i, "ETDPASI")), "NULL", "'" & grid.GetRowValues(i, "ETDPASI") & "'") & ") " & vbCrLf & _
                                  " END " & vbCrLf & _
                                  " ELSE " & vbCrLf & _
                                  " BEGIN" & vbCrLf & _
                                  "      UPDATE [dbo].[MS_ETD_PASI] SET " & vbCrLf & _
                                  "            [ETDPASI] = " & IIf(IsDBNull(grid.GetRowValues(i, "ETDPASI")), "NULL", "'" & grid.GetRowValues(i, "ETDPASI") & "'") & " " & vbCrLf & _
                                  "      WHERE [AffiliateID] = '" & grid.GetRowValues(i, "AffiliateID") & "' and [ETAAffiliate] = '" & grid.GetRowValues(i, "ETAAffiliate") & "'" & vbCrLf & _
                                  " END"
