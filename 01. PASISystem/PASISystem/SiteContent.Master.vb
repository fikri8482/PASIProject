        ...
        Try
            Using sqlConn As New SqlConnection(clsGlobal.ConnectionString)
                ...
                With tvMenu
                    ...
                    '          "  " & vbCrLf & _
                    '          " set @StatAdmin = (select StatusAdmin from SC_UserSetup where UserID = '" & Session("UserID").ToString() & "' and UserCls = '1') " & vbCrLf & _
                    '          "  " & vbCrLf & _
                    ...
                    '          " 		where b.UserID = '" & Session("UserID").ToString() & "' and a.PASIMenu = '1' and UserCls = '1'" & vbCrLf & _
                    ...
                    ls_SQL = " 		select distinct a.GroupIndex, a.GroupID from SC_UserMenu a " & vbCrLf & _
                              " 		inner join SC_UserPrivilege b on a.AppID = b.AppID and a.MenuID = b.MenuID " & vbCrLf & _
                              " 		where b.UserID = '" & Session("UserID").ToString() & "' and a.PASIMenu in ('1','2') and UserCls = '1' and AllowAccess = '1'" & vbCrLf & _
                    ...
                    If dsParent.Tables(0).Rows.Count > 0 Then
                        For iLoopParent = 0 To dsParent.Tables(0).Rows.Count - 1
                            ...
                            '          " 		WHERE b.UserID = '" & Session("UserID").ToString() & "' and b.AllowAccess = '1' AND a.GroupID = @GroupID AND a.PASIMenu = '1' and UserCls = '1' and a.ShowCls = '1'" & vbCrLf & _
                            '          " 		ORDER BY a.MenuID "
                            ...
                            'ls_SQL = ls_SQL + "  				where GroupID = 'Security System' and GroupIndex is not null and b.UserCls = '1' and a.ShowCls = '1' order by a.MenuID" & vbCrLf & _
                            '                  "  			end " & vbCrLf & _
                            '                  "  		END  "
                            ls_SQL = " declare @StatAdmin char(1) " & vbCrLf & _
                                      " declare @GroupID char(50)" & vbCrLf & _
                                      ...
                                      "  				where  b.UserID = '" & Session("UserID").ToString() & "' and  GroupID = 'Security System' and GroupIndex is not null and b.UserCls = '1' and a.ShowCls = '1' and AllowAccess = '1' order by a.MenuID" & vbCrLf & _
                                      "  			end " & vbCrLf
...
#Region "FORM EVENTS"
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        Try
            If Session("UserID") = "" And Page.IsCallback = False Then Session("Msg") = "Please Login First!" : Session("GlobalURL") = Request.RawUrl : Response.Redirect("~/Login.aspx") : Exit Sub

            'lblTitle.Text = "EBWEB - PT. AUTOCOMP SYSTEM INDONESIA - " & "[" & clsGlobal.GetMenuID(Session("MenuDesc")) & "] " & Session("MenuDesc")
            lblTitle.Text = "PURCHASING SYSTEM - PASI"
            ...
            If Not Page.IsPostBack Then
                ...
                ASPxSplitter1.Panes("Menu").Size = 350
                Call LoadMainMenu()
        ...
        Catch ex As Exception
            ...
            MsgBox("[" & Err.Number & "] " & Err.Description, MsgBoxStyle.Critical, "PASI System")
        End Try
    End Sub
