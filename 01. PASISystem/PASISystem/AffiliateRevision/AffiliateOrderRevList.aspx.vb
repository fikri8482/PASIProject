...
Public Class AffiliateOrderRevList
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If (Not IsPostBack) AndAlso (Not IsCallback) Then
            If Session("M01Url") <> "" Then
                Session("MenuDesc") = "AFFILIATE ORDER REVISION LIST"
...
#Region "PROCEDURE"
    Private Sub bindData(ByVal pDateFrom As Date, ByVal pDateTo As Date, ByVal pPORevNo As String, ByVal pPONo As String, ByVal pAff As String, ByVal pSupp As String, ByVal pSend As String, ByVal pComm As String)
        ...
        Using sqlConn As New SqlConnection(clsGlobal.ConnectionString)
            ...
            ls_SQL = "    SELECT row_number() over (order by PORM.PONo) as NoUrut, PORM.Period,PORM.AffiliateID,AffiliateName,RTRIM(PORM.PORevNo)PORevNo,RTRIM(PORM.PONo)PONo " & vbCrLf & _
                      "    ,CASE WHEN ISNULL(POM.CommercialCls,0) = 0 THEN 'NO' ELSE 'YES' END CommercialCls ,PORM.SeqNo " & vbCrLf & _
                      ...
                      "    ,KanbanCls,PORM.EntryDate,PORM.EntryUser  " & vbCrLf & _                      
            ...
            ls_SQL = ls_SQL + "    ,CASE WHEN ISNULL(PORM.SupplierApprovePendingUser,'') <> '' OR ISNULL(PORM.SupplierApprovePendingDate,'') <> ''  THEN 1 ELSE 0 END POStatus5    " & vbCrLf & _
                              ...
                              "    LEFT JOIN dbo.PO_Detail POD ON POM.AffiliateID = POD.AffiliateID AND POM.PONo = POD.PONo AND POM.SupplierID = POD.SupplierID" & vbCrLf & _
            ...
            ls_SQL = ls_SQL + "    WHERE MONTH(PORM.Period) BETWEEN MONTH('" & Format(pDateFrom, "yyyy-MM-dd") & "') AND MONTH('" & Format(pDateTo, "yyyy-MM-dd") & "') " & vbCrLf & _
                              "    AND YEAR(PORM.Period) BETWEEN YEAR('" & Format(pDateFrom, "yyyy-MM-dd") & "') AND YEAR('" & Format(pDateTo, "yyyy-MM-dd") & "') " & vbCrLf & _
                              "    AND ISNULL(PORM.AffiliateApproveUser,'')  <> ''  " & vbCrLf
            ...
            Select Case pSend
                Case "0"
                    ls_SQL = ls_SQL + " AND ISNULL(PORM.PASISendAffiliateDate,'') = '' " & vbCrLf
                Case "1"
                    ls_SQL = ls_SQL + " AND ISNULL(PORM.PASISendAffiliateDate,'') <> ''" & vbCrLf
            End Select
            ls_SQL = ls_SQL + "   GROUP BY PORM.Period,PORD.PONo,PORM.PORevNo,PORM.PONo,PORM.AffiliateID,AffiliateName,POM.CommercialCls,PORM.SupplierID,SupplierName,POM.ShipCls,PORD.SupplierID " & vbCrLf & _
                              "    ,KanbanCls ,PORM.SeqNo,PORM.EntryDate,PORM.EntryUser,PORM.AffiliateApproveUser ,PORM.AffiliateApproveDate    " & vbCrLf & _
                              ...
                              "    ,PORM.SupplierUnApproveUser,PORM.SupplierUnApproveDate,PORM.PASIApproveUser,PORM.PASIApproveDate,PORM.FinalApproveUser,PORM.FinalApproveDate " & vbCrLf 
    ...
    Private Sub up_GridLoadWhenEventChange()
        ...
        Using sqlConn As New SqlConnection(clsGlobal.ConnectionString)
            ...
            ls_SQL = " select top 0  '' DetailPage, '' as PORevNo, ''POMarking, '' Period, ''PONo, ''CommercialCls, ''ShipCls, ''CurrAff, ''AmountAff, '' EntryDate, ''EntryUser, '' POStatus1, ''POStatus2, ''POStatus3, ''POStatus4, ''POStatus5, ''POStatus6, ''POStatus7, ''POStatus8"
    ...
    Protected Function GetShip(ByVal container As GridViewDataItemTemplateContainer) As String
        GetShip = container.Grid.GetRowValues(container.ItemIndex, "ShipCls")
    End Function

    Protected Function GetSeq(ByVal container As GridViewDataItemTemplateContainer) As String
        GetSeq = container.Grid.GetRowValues(container.ItemIndex, "SeqNo")
    End Function
#End Region
