...
#Region "FORM EVENTS"
    ...
    Private Sub cboUserGroup_Callback(sender As Object, e As DevExpress.Web.ASPxClasses.CallbackEventArgsBase) Handles cboUserGroup.Callback
        Try
            Call up_FillCombo(IIf(cboAffiliateID.Text.Trim = "PASI", "1", "0"))
    ...
    Private Sub gridUser_CustomCallback(ByVal sender As Object, ByVal e As DevExpress.Web.ASPxGridView.ASPxGridViewCustomCallbackEventArgs) Handles gridUser.CustomCallback
        ...
        Dim pUserCls As String = IIf(cboAffiliateID.Text = "PASI", 1, 0)
        Try
            Select Case pAction
                Case "load"
                    Call up_GridLoadUser()
                ...
                Case "save"
                    ...
                    If txtCCTemp.Text = "" Then
                        lb_IsUpdate = validasiInput(pUserCls)                    
                    End If

                    If lb_IsUpdate = False Then
                    ...
                    If ls_userPr <> "" Then
                        Call up_SaveDataByUP(ls_userPr, Split(e.Parameters, "|")(9))                    
                    End If
                    Call up_GridLoadUser()
                    Call up_GridLoadPrivilege(False)

                Case "delete"
                    Call up_DeleteDataPP(Split(e.Parameters, "|")(2), pUserCls)
                    Call up_DeleteData(Split(e.Parameters, "|")(1), _
                                       Split(e.Parameters, "|")(2), pUserCls)
                    txtUserIDTemp.Text = ""
                    txtCCTemp.Text = ""
                    txtFullName.Text = ""
                    txtPasswordUS.Text = ""
        ...
        Using sqlConn As New SqlConnection(clsGlobal.ConnectionString)
            ...
            Using sqlTran As SqlTransaction = sqlConn.BeginTransaction("UserMenu")
                ...
                For iLoop = 0 To a - 1
                    ...
                    ls_SQL = " IF NOT EXISTS (SELECT * FROM dbo.SC_UserPrivilege WHERE UserID ='" & ls_UserID & "' AND MenuID='" & ls_MenuID & "' AND UserCls = '" & ls_UserCls & "')" & vbCrLf & _
                              " BEGIN " & vbCrLf & _
                            " INSERT INTO dbo.SC_UserPrivilege( AppID, UserCls ,UserID ,MenuID ,AllowAccess ,AllowUpdate, AllowDelete,AllowConfirm) " & vbCrLf & _
                            " VALUES( 'P01', '" & ls_UserCls & "' ," & vbCrLf & _
                            " '" & ls_UserID & "' ," & vbCrLf & _
                            " '" & ls_MenuID & "' ," & vbCrLf & _
                    ...
                    ls_SQL = ls_SQL + " UPDATE dbo.SC_UserPrivilege " & vbCrLf & _
                          ...
                          " WHERE AppID='P01' AND UserID='" & ls_UserID & "' AND MenuID='" & ls_MenuID & "' AND UserCls = '" & ls_UserCls & "'" & vbCrLf & _
                          " END "
                    ls_MsgID = "1002"
    ...
    Private Sub gridMenu_PageIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles gridMenu.PageIndexChanged
        Dim ls_UserCls As String = IIf(cboAffiliateID.Text = "PASI", 1, 0)
        If cboUserGroup.Text <> "" Then
            up_GridLoadMenuCombo(ls_UserCls)
        Else
            up_GridLoadMenu(ls_UserCls)
        End If
    End Sub
        ...
        Using sqlConn As New SqlConnection(clsGlobal.ConnectionString)
            ...
            If txtUserIDTemp.Text <> "" Then
                ls_SQL = " SELECT RTRIM(AffiliateID) AffiliateID,RTRIM(UserID) UserID,RTRIM(FullName)FullName,Password,InvalidLogin,Locked,StatusAdmin,RTRIM(Description)Description FROM dbo.SC_UserSetup " & vbCrLf & _
                    ...
                    " AND UserID='" & Trim(txtUserId.Text) & "' AND UserCls = '" & ls_UserCls & "' " & vbCrLf & _
                    " ORDER BY UserID  "
...
#Region "PROCEDURE"
    ...
    Public Sub up_GridLoadPrivilege(ByVal combo As Boolean)
        Dim ls_UserCls As String = IIf(cboAffiliateID.Text = "PASI", 1, 0)
        If combo = True Then
            Call up_GridLoadMenuCombo(ls_UserCls)
        Else
            Call up_GridLoadMenu(ls_UserCls)
        End If
        txtCCTemp.ForeColor = Color.FromName("#96C8FF")
        txtUserIDTemp.ForeColor = Color.FromName("#96C8FF")
    End Sub

    Public Sub up_GridLoadMenu(Optional ByVal pPASICls As String = "")
        Dim ls_SQL As String = ""
        Dim ls_UserID As String = ""
        ...
        Using sqlConn As New SqlConnection(clsGlobal.ConnectionString)
            ...
            If ls_UserID = "" Then
                ls_SQL = "  SELECT GroupID, USM.MenuID, MenuDesc,   " & vbCrLf & _
                  "  0 as AllowAccess,  " & vbCrLf & _
                  "  0 as AllowUpdate,  " & vbCrLf & _
                  "  0 as AllowDelete,  " & vbCrLf & _
                  "  0 as AllowConfirm   " & vbCrLf & _
                  "  FROM SC_UserMenu USM " & vbCrLf & _
                  "  --LEFT JOIN (Select * from SC_UserPrivilege where UserID='" & ls_UserID & "' AND UserCls = '" & pPASICls & "' ) UP   " & vbCrLf & _
                  "  --ON USM.AppID = UP.AppID and USM.MenuID=UP.MenuID    " & vbCrLf & _
                  "  WHERE USM.ShowCls ='1' and USM.AppID='P01' and GroupIndex is not null and PASIMenu IN ('" & pPASICls & "','2') " & ls_Where & "" & vbCrLf & _
                  "  ORDER by USM.MenuID  "
            Else
                ls_SQL = "  SELECT GroupID, USM.MenuID, MenuDesc,   " & vbCrLf & _
    ...
    Public Sub up_GridLoadMenuCombo(Optional ByVal pPASICls As String = "")
        Dim ls_SQL As String = ""
        Dim ls_UserID As String = ""
        Dim ls_Where As String = ""
    ...
    Public Sub up_SaveData(ByVal pIsUpdate As Boolean, _
                            ...
                            Optional ByVal pDesc As String = "", _
                            Optional ByVal pUserCls As String = "")
            ...
            Using sqlTran As SqlTransaction = sqlConn.BeginTransaction("UserSetup")
                If pIsUpdate = True Then
                    ...
                    ls_SQL = " INSERT INTO dbo.SC_UserSetup " & vbCrLf & _
                             ...
                             " VALUES ('P01','" & Trim(pCCCode) & "', '" & pUserCls & "' ,'" & Trim(pUserID) & "','" & Trim(pFullName) & "','" & a & "'," & vbCrLf & _
                ...
                Else
                    ls_SQL = " UPDATE dbo.SC_UserSetup " & vbCrLf & _
                             ...
                             " AffiliateID='" & Trim(pCCCode) & "' AND UserCls = '" & pUserCls & "'"
    ...
    Public Sub up_SaveDataByUP(ByVal pbyUserID As String, ByVal pUserCls As String)
        Dim ls_SQL As String = "", ls_MenuID As String = "", ls_MsgID As String = ""
        Dim iLoop As Long = 0, jLoop As Long = 0
        ...
        Using sqlConn As New SqlConnection(clsGlobal.ConnectionString)
            ...
            ls_SQL = "  SELECT GroupID, USM.MenuID, MenuDesc,   " & vbCrLf & _
                  ...
                  "  ISNULL(AllowConfirm,'0') as AllowConfirm   " & vbCrLf & _
                  "  FROM SC_UserMenu USM " & vbCrLf & _
                  "  LEFT JOIN (Select * from SC_UserPrivilege where UserID='" & pbyUserID & "' AND UserCls = '" & pUserCls & "' ) UP   " & vbCrLf & _
                  "  ON USM.AppID = UP.AppID and USM.MenuID=UP.MenuID    " & vbCrLf & _
                  "  WHERE USM.AppID='P01' and GroupIndex is not null and PASIMenu IN ('" & pUserCls & "','2') " & vbCrLf & _
                  "  ORDER by USM.MenuID  "
            ...
            If ds.Tables(0).Rows.Count > 0 Then
                With ds.Tables(0)
                    For iLoop = 0 To .Rows.Count - 1
                        ...
                        'TRANSACTION PROCESS
                        ls_SQL = " IF NOT EXISTS (SELECT * FROM dbo.SC_UserPrivilege WHERE UserID ='" & ls_UserID & "' AND MenuID='" & ls_MenuID & "' AND UserCls = '" & pUserCls & "')" & vbCrLf & _
                          " BEGIN " & vbCrLf & _
                        " INSERT INTO dbo.SC_UserPrivilege( AppID, UserCls ,UserID ,MenuID ,AllowAccess ,AllowUpdate, AllowDelete ,AllowConfirm) " & vbCrLf & _
                        " VALUES( 'P01', '" & pUserCls & "' ," & vbCrLf & _
                        " '" & ls_UserID & "' ," & vbCrLf & _
                        " '" & ls_MenuID & "' ," & vbCrLf & _
                        " '" & ls_AllowAccess & "' ," & vbCrLf & _
                        " '" & ls_AllowUpdate & "' ," & vbCrLf & _
                        ...
                        ls_SQL = ls_SQL + " UPDATE dbo.SC_UserPrivilege " & vbCrLf & _
                              ...
                              " AllowDelete='" & ls_AllowDelete & "', " & vbCrLf & _
                              " AllowConfirm='" & ls_AllowConfirm & "' " & vbCrLf & _
                              " WHERE AppID='P01' AND UserID='" & ls_UserID & "' AND MenuID='" & ls_MenuID & "' AND UserCls = '" & pUserCls & "'" & vbCrLf & _
                              " END "
    ...
    '                    ls_SQL = " IF NOT EXISTS (SELECT * FROM dbo.SC_UserPrivilege WHERE UserID ='" & ls_UserID & "' AND MenuID='" & ls_MenuID & "' AND UserCls = '1')" & vbCrLf & _
    '                          " BEGIN " & vbCrLf & _
    '                        " INSERT INTO dbo.SC_UserPrivilege( AppID, UserCls ,UserID ,MenuID ,AllowAccess ,AllowUpdate, AllowDelete ,AllowSpecial) " & vbCrLf & _
    '                        " VALUES( 'P01', '1' ," & vbCrLf & _
    '                        " '" & ls_UserID & "' ," & vbCrLf & _
    '                        " '" & ls_MenuID & "' ," & vbCrLf & _
    '                        " '" & ls_AllowAccess & "' ," & vbCrLf & _
    '                        " '" & ls_AllowUpdate & "' ," & vbCrLf & _
    ...
    '                          " WHERE AppID='P01' AND UserID='" & ls_UserID & "' AND MenuID='" & ls_MenuID & "' AND UserCls = '1'" & vbCrLf & _
    '                          " END "
    ...
    Private Sub up_DeleteData(ByVal pCCCode As String, ByVal pUserID As String, ByVal pUserCls As String)
        Dim ls_SQL As String = ""
        ...
        Using sqlConn As New SqlConnection(clsGlobal.ConnectionString)
            ...
            Using sqlTran As SqlTransaction = sqlConn.BeginTransaction("FAType")
                ...
                ls_SQL = "DELETE dbo.SC_UserSetup WHERE AffiliateID ='" & pCCCode & "' AND UserID='" & pUserID & "' AND UserCls = '" & pUserCls & "'"
                Dim sqlComm As New SqlCommand(ls_SQL, sqlConn, sqlTran)
                sqlComm.ExecuteNonQuery()
    ...
    Private Sub up_DeleteDataPP(ByVal pUserID As String, ByVal pUserCls As String)
        Dim ls_SQL As String = ""
        ...
        Using sqlConn As New SqlConnection(clsGlobal.ConnectionString)
            ...
            Using sqlTran As SqlTransaction = sqlConn.BeginTransaction("User")
                ...
                ls_SQL = "DELETE FROM dbo.SC_UserPrivilege WHERE UserID='" & pUserID & "' AND UserCls = '" & pUserCls & "'"
                Dim sqlComm As New SqlCommand(ls_SQL, sqlConn, sqlTran)
                sqlComm.ExecuteNonQuery()
    ...
    Private Sub up_FillCombo(ByVal pAffiliate As String)
        Dim ls_SQL As String = ""

        ls_SQL = "SELECT rtrim(UserID)UserID, rtrim(FullName)FullName FROM dbo.SC_UserSetup WHERE UserCls = '" & pAffiliate & "' ORDER BY UserID" & vbCrLf
        Using sqlConn As New SqlConnection(clsGlobal.ConnectionString)
            sqlConn.Open()
    ...
    Private Function validasiInput(ByVal pUserCls As String) As Boolean
        validasiInput = True
        Try
            ...
            Using sqlConn As New SqlConnection(clsGlobal.ConnectionString)
                ...
                'sqlstring = "SELECT * FROM dbo.SC_UserSetup WHERE AppID='P01' AND UserID='" & Trim(txtUserId.Text) & "' AND AffiliateID='" & Trim(cboAffiliateID.Text) & "' AND UserCls = '" & pUserCls & "'"
                sqlstring = "SELECT * FROM dbo.SC_UserSetup WHERE AppID='P01' AND UserID='" & Trim(txtUserId.Text) & "'"
                ...
                If ds.Tables(0).Rows.Count > 0 Then
                    ...
                    lblErrMsg.Text = "[9999] User ID with ID """ & Trim(txtUserId.Text) & """ already exists in system"
                    txtUserId.Focus()
                    Return False
                End If
                Return True
