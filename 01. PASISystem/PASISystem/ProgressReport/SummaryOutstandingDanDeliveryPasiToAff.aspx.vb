...
Public Class SummaryOutstandingDanDeliveryPasiToAff
...
#Region "Procedures"
    ...
    Private Sub up_GridLoad()
        Using sqlConn As New SqlConnection(clsGlobal.ConnectionString)
            ...
            If chkSupplierDelDate.Checked = True Then
                ls_filter = ls_filter + _
                              "                      AND CONVERT(date,DOM.DeliveryDate) BETWEEN '" & Format(dtSupplierDelDateFrom.Value, "yyyy-MM-dd") & "' AND '" & Format(dtSupplierDelDateTo.Value, "yyyy-MM-dd") & "' " & vbCrLf
            End If
            ...
            ls_SQL = "  SELECT *  " & vbCrLf &
                     "  ,StatusPO = CASE WHEN Remaining1 > 0 THEN 'OVER'  " & vbCrLf &
                     "  WHEN Remaining1 < 0 THEN 'OPEN'  " & vbCrLf &
                     "  ELSE 'CLOSE' END, Remarks = Remarksxx  " & vbCrLf &
                     "  FROM (  " & vbCrLf &
                     "  SELECT    " & vbCrLf &
                     "   	POM.Period   " & vbCrLf &
                     "   	,POM.PONo   " & vbCrLf &
                     "   	,POM.AffiliateID   " & vbCrLf &
                     "   	,POM.SupplierID   " & vbCrLf &
                     "   	,POKanban = CASE WHEN ISNULL(POD.KanbanCls, '0') = '1' THEN 'YES' ELSE 'NO' END   " & vbCrLf

            ls_SQL = ls_SQL + "   	,POM.PASISendAffiliateDate   " & vbCrLf &
                              "   	,POD.PartNo   " & vbCrLf &
                              "   	,MP.PartName   " & vbCrLf &
                              "   	,QtyPO = ISNULL(POD.POQty,0)   " & vbCrLf &
                              "  	,QtyBox = ISNULL(POD.POQtyBox,MPM.QtyBox)  " & vbCrLf &
                              "  	,MPM.BoxPallet  " & vbCrLf &
                              "  	,VolumePallet = CASE WHEN ISNULL(MPM.BoxPallet,0) = 0 Then 0 ELSE (POD.POQty/ISNULL(POD.POQtyBox,MPM.QtyBox))/MPM.BoxPallet End " & vbCrLf &
                              "   	,ETDSupp = ES.ETDSupplier   " & vbCrLf &
                              "   	,ETAAff = KM.KanbanDate   " & vbCrLf &
                              "   	,PASIDeliveryDate = DOM.DeliveryDate   " & vbCrLf &
                              "   	,PASISuratJalanNo = DOM.SuratJalanNo " & vbCrLf &
                              "   	,Remarksxx = ISNULL((SELECT TOP 1 CONCAT(Case When ISNULL(Commercial, '') IN ('') Then 'COMMERCIAL' Else Commercial End,' VALUE') FROM PLPASI_Master WHERE AffiliateID = POM.AffiliateID And SuratJalanNo = DOM.SuratJalanNo), '') " & vbCrLf

            ls_SQL = ls_SQL + "     ,PASIInvoiceNo =  CASE WHEN (SELECT TOP 1 CASE WHEN InvoiceNo Is Not NULL THEN InvoiceNo ELSE SuratJalanNo END AS InvoiceNo FROM PLPASI_Master WHERE AffiliateID=POM.AffiliateID And  SuratJalanNo=DOM.SuratJalanNo) Is Not NULL THEN (SELECT TOP 1 CASE WHEN InvoiceNo Is Not NULL THEN InvoiceNo ELSE SuratJalanNo END AS InvoiceNo FROM PLPASI_Master WHERE AffiliateID=POM.AffiliateID And  SuratJalanNo=DOM.SuratJalanNo) ELSE DOM.SuratJalanNo END     " & vbCrLf &
                              "     --,PASIInvoiceDate = CASE WHEN (SELECT TOP 1 CASE WHEN InvoiceDate Is Not NULL THEN InvoiceDate ELSE InvoiceDate END AS InvoiceDate FROM PLPASI_Master WHERE AffiliateID=POM.AffiliateID And  SuratJalanNo=DOM.SuratJalanNo) Is Not NULL THEN (SELECT TOP 1 CASE WHEN InvoiceDate Is Not NULL THEN InvoiceDate ELSE InvoiceDate END AS InvoiceDate FROM PLPASI_Master WHERE AffiliateID=POM.AffiliateID And  SuratJalanNo=DOM.SuratJalanNo) ELSE DOM.InvoiceDate END   " & vbCrLf &
                              ",PASIInvoiceDate = " & vbCrLf &
                              "--Case " & vbCrLf &
                              "--When (SELECT TOP 1 Format(InvoiceDate, 'yyyy-MM-dd') FROM PLPASI_Master WHERE AffiliateID = POM.AffiliateID And SuratJalanNo = DOM.SuratJalanNo) IS NULL Then '' " & vbCrLf &
                              "--Else (SELECT TOP 1 Format(InvoiceDate, 'yyyy-MM-dd') FROM PLPASI_Master WHERE AffiliateID = POM.AffiliateID And SuratJalanNo = DOM.SuratJalanNo)" & vbCrLf &
                              "--End " & vbCrLf &
                              "Case " & vbCrLf &
                              "When (SELECT TOP 1 InvoiceDate FROM PLPASI_Master WHERE AffiliateID = POM.AffiliateID And SuratJalanNo = DOM.SuratJalanNo) IS NULL Then NULL " & vbCrLf &
                              "Else (SELECT TOP 1 InvoiceDate FROM PLPASI_Master WHERE AffiliateID = POM.AffiliateID And SuratJalanNo = DOM.SuratJalanNo)" & vbCrLf &
                              "End " & vbCrLf &
                              " 	,PASIDeliveryQty = DOD.DOQty " & vbCrLf &
                              "     ,AffiliateReceiveDate = RM.ReceiveDate " & vbCrLf &
                              "     ,AffiliateReceiveQty = ISNULL(RD.RecQty,0) " & vbCrLf &
                              "  	,Remaining = (ISNULL(POD.POQty,0) -  " & vbCrLf &
                              "      ( " & vbCrLf &
                              "          Select ISNULL(SUM(RecQty), 0)  " & vbCrLf &
                              "          FROM ReceiveAffiliate_Detail RRD  " & vbCrLf &
                              "          LEFT JOIN ReceiveAffiliate_Master RRM ON RRM.AffiliateID = RRD.AffiliateID  " & vbCrLf &
                              "                AND RRM.SuratJalanNo = RRD.SuratJalanNo " & vbCrLf &
                              "                AND RRM.SupplierID = RRD.SupplierID  " & vbCrLf &
                              "          WHERE RRD.PONO = POD.PONo " & vbCrLf &
                              "            AND RRD.SupplierID = POD.SupplierID " & vbCrLf

            ls_SQL = ls_SQL + "            AND RRD.AffiliateID = POD.AffiliateID " & vbCrLf & _
                              "            AND RRD.PartNo = POD.PartNo " & vbCrLf & _
                              "            AND RRM.ReceiveDate <= RM.ReceiveDate " & vbCrLf & _
                              "      )) * (-1) " & vbCrLf & _
                              "  	,Remaining1 = (ISNULL(POD.POQty,0) -  " & vbCrLf & _
                              "      ( " & vbCrLf & _
                              "          Select ISNULL(SUM(RecQty), 0)  " & vbCrLf & _
                              "          FROM ReceiveAffiliate_Detail RRD  " & vbCrLf & _
                              "          LEFT JOIN ReceiveAffiliate_Master RRM ON RRM.AffiliateID = RRD.AffiliateID  " & vbCrLf & _
                              "                AND RRM.SuratJalanNo = RRD.SuratJalanNo " & vbCrLf & _
                              "                AND RRM.SupplierID = RRD.SupplierID  " & vbCrLf & _
                              "          WHERE RRD.PONO = POD.PONo " & vbCrLf & _
                              "            AND RRD.SupplierID = POD.SupplierID " & vbCrLf
            ...
            ls_SQL = ls_SQL + "            AND RRD.AffiliateID = POD.AffiliateID " & vbCrLf & _
                              ...
                              "            --AND RRM.ReceiveDate <= RM.ReceiveDate " & vbCrLf & _
                              "      )) * (-1) " & vbCrLf & _
                              "  FROM (Select PoNo,AffiliateID,SupplierID,Period,PASISendAffiliateDate " & vbCrLf & _
                              "        From PO_Master WITH(NOLOCK) " & vbCrLf & _
                              "  Where 'A' = 'A' " & vbCrLf
            If chkPOPeriod.Checked = True Then
                ls_SQL = ls_SQL + _
                              "        And CONVERT(date,Period) BETWEEN '" & Format(dtPOPeriodFrom.Value, "yyyy-MM-01") & "' AND '" & Format(dtPOPeriodTo.Value, "yyyy-MM-" & ls_End) & "' " & vbCrLf
            End If
            If Trim(cboAffiliateCode.Text) <> "==ALL==" And Trim(cboAffiliateCode.Text) <> "" Then
                ls_SQL = ls_SQL + _
            ...
            ls_SQL = ls_SQL + "       ) POM   " & vbCrLf & _
                              ...
                              "  LEFT JOIN (Select AffiliateID,PoNo,KanbanNo,SupplierID,DeliveryLocationCode,PartNo,KanbanQty " & vbCrLf & _
                              "             From Kanban_Detail " & vbCrLf & _
                              "             WHERE KanbanQty > 0 " & vbCrLf & _
                              "  ) KD ON KD.AffiliateID = POD.AffiliateID   " & vbCrLf & _
            ...
            ls_SQL = ls_SQL + "   							AND KD.PartNo = POD.PartNo   " & vbCrLf & _
                              ...
                              " 	LEFT JOIN DOPASI_Detail DOD ON KD.AffiliateID = DOD.AffiliateID " & vbCrLf & _
                              ...
                              " 	LEFT JOIN DOPASI_Master DOM ON DOM.AffiliateID = DOD.AffiliateID   " & vbCrLf

            ls_SQL = ls_SQL + "   								AND DOM.SuratJalanNo = DOD.SuratJalanNo   " & vbCrLf & _
                              "   								AND DOM.SupplierID = DOD.SupplierID " & vbCrLf & _
                              "     LEFT JOIN ReceiveAffiliate_Master RM ON DOM.AffiliateID = RM.AffiliateID " & vbCrLf & _
                              "			                                AND DOM.SupplierID = RM.SupplierID " & vbCrLf & _
                              "                                         AND DOM.SuratJalanNo = RM.SuratJalanNo " & vbCrLf & _
                              "     LEFT JOIN ReceiveAffiliate_Detail RD ON RM.AffiliateID = RD.AffiliateID " & vbCrLf & _
                              "                                         AND RM.SupplierID = RD.SupplierID " & vbCrLf & _
                              "                                         AND RM.SuratJalanNo = RD.SuratJalanNo " & vbCrLf & _
                              ...
                              "  	LEFT JOIN MS_ETD_Supplier_PASI ES ON POM.SupplierID = ES.SupplierID  " & vbCrLf & _
                              "  										AND EP.ETDPASI =  ES.ETAPASI   " & vbCrLf & _
            ...
            ls_SQL = ls_SQL + ")A " & vbCrLf & _
                              " Order By PASISuratJalanNo " & vbCrLf
    ...
    Private Function GetSummaryOutStanding() As DataTable
        ...
        Try
            ...
            Using cn As New SqlConnection(clsGlobal.ConnectionString)
                ...
                ls_sql = "  SELECT Period,PONo,AffiliateID,SupplierID,POKanban,PASISendAffiliateDate,PartNo,PartName,QtyPO,QtyBox,BoxPallet  " & vbCrLf &
                         " ,VolumePallet,ETDSupp,ETAAff,PASIDeliveryDate,PASISuratJalanNo,PASIInvoiceNo,PASIInvoiceDate,PASIDeliveryQty,AffiliateReceiveDate,AffiliateReceiveQty,Remaining" & vbCrLf &
                         "  ,StatusPO = CASE WHEN Remaining1 > 0 THEN 'OVER'  " & vbCrLf &
                         "  WHEN Remaining1 < 0 THEN 'OPEN'  " & vbCrLf &
                         "  ELSE 'CLOSE' END, Remarks  " & vbCrLf &
                         "  FROM (  " & vbCrLf &
                         "  SELECT    " & vbCrLf &
                         "   	POM.Period   " & vbCrLf &
                         "   	,PONo=POM.PONo " & vbCrLf &
                ...
                ls_sql = ls_sql + "   	,POM.PASISendAffiliateDate   " & vbCrLf &
                                  ...
                                  "   	,PASIDeliveryDate = DOM.DeliveryDate   " & vbCrLf &
                                  "   	,PASISuratJalanNo = DOM.SuratJalanNo " & vbCrLf &
                                  "   	,Remarks = ISNULL((SELECT TOP 1 CONCAT(Case When ISNULL(Commercial, '') IN ('') Then 'COMMERCIAL' Else Commercial End,' VALUE') FROM PLPASI_Master WHERE AffiliateID = POM.AffiliateID And SuratJalanNo = DOM.SuratJalanNo), '') " & vbCrLf
    ...
    Private Sub up_GridLoadWhenEventChange()
        Using sqlConn As New SqlConnection(clsGlobal.ConnectionString)
            ...
            ls_SQL = ls_SQL + " 	,BoxPallet = '' " & vbCrLf &
                              " 	,VolumePallet = '' " & vbCrLf &
                              "  	,ETDSupp = '' " & vbCrLf &
                              "  	,ETAAff = '' " & vbCrLf &
    ...
    Private Sub epplusExportExcel(ByVal pFilename As String, ByVal pSheetName As String,
        ...
        Try
            Dim tempFile As String = "TemplateSummaryOutstandingDeliveryPasiToAffiliate " & Format(Now, "yyyyMMdd hhmmss") & ".xlsx"
            ...
            With ws
                ...
                .Cells(8, 20, pData.Rows.Count + 7, 20).Style.Numberformat.Format = "dd-mmm-yy"
                ...
                '.Cells(8, 18, pData.Rows.Count + 7, 19).Style.Numberformat.Format = "#,##0"
                ...
                Dim rgAll As ExcelRange = .Cells(8, 1, pData.Rows.Count + 7, 24)
        ...
        Try
            ...
            Select Case pAction
                ...
                Case "excel"
                    ...
                    FileName = "TemplateSummaryOutstandingDeliveryPasiToAffiliate.xlsx"
