...
Public Class SummaryOutstandingDanDeliverySupplier
...
#Region "Procedures"
    ...
    Private Sub up_GridLoad()
        Using sqlConn As New SqlConnection(clsGlobal.ConnectionString)
            ...
            ls_SQL = ls_SQL + " SELECT   " & vbCrLf & _
                              ...
                              " 	,ISNULL(POD.POQtyBox,MPM.QtyBox) QtyBox " & vbCrLf

            ls_SQL = ls_SQL + " 	,MPM.BoxPallet " & vbCrLf & _
                              " 	,VolumePallet = case when isnull(POD.POQty,0) = 0 then 0 when ISNULL(POD.POQtyBox,MPM.QtyBox) = 0 then 0 when isnull(mpm.boxpallet,0) = 0 then 0 else (POD.POQty/ISNULL(POD.POQtyBox,MPM.QtyBox))/MPM.BoxPallet End " & vbCrLf & _
                              ...
                              "     (" & vbCrLf & _
                              "         Select ISNULL(SUM(GoodRecQty), 0) " & vbCrLf & _
                              "         FROM ReceivePASI_Detail RD " & vbCrLf & _
                              "         LEFT JOIN ReceivePASI_Master RM ON RM.AffiliateID = RD.AffiliateID  " & vbCrLf & _
                              "               AND RM.SuratJalanNo = RD.SuratJalanNo  " & vbCrLf & _
                              "               AND RM.SupplierID = RD.SupplierID " & vbCrLf & _
                              "         WHERE RD.PONO = POD.PONo" & vbCrLf & _
                              "           AND RD.SupplierID = POD.SupplierID" & vbCrLf & _
                              "           AND RD.AffiliateID = POD.AffiliateID" & vbCrLf & _
                              "           AND RD.PartNo = POD.PartNo" & vbCrLf & _
                              "           AND RM.ReceiveDate <= PRM.ReceiveDate" & vbCrLf & _
                              "     )) * (-1)" & vbCrLf & _
            ...
            ls_SQL = ls_SQL + " 	LEFT JOIN MS_ETD_PASI EP ON POM.AffiliateID = EP.AffiliateID " & vbCrLf & _
    ...
    Private Function GetSummaryOutStanding() As DataTable
        ...
        Try
            ...
            Using cn As New SqlConnection(clsGlobal.ConnectionString)
                ...
                ls_sql = " SELECT Period,PONo,AffiliateID,SupplierID,POKanban,PASISendAffiliateDate,PartNo,PartName,QtyPO,QtyBox,BoxPallet,VolumePallet " & vbCrLf & _
                         " ,ETDSupp,ETAAff,SupplierDeliveryDate,SupplierSuratJalanNo,SupplierDeliveryQty,PASIReceiveDate,PASIReceivingQty,Remaining" & vbCrLf & _
                     " ,StatusPO = CASE WHEN Remaining1 > 0 THEN 'OVER' " & vbCrLf & _
                     " WHEN Remaining1 < 0 THEN 'OPEN' " & vbCrLf & _
                     " ELSE 'CLOSE' END " & vbCrLf & _
                     " FROM ( " & vbCrLf
    ...
    Private Function Trimdata(dt As DataTable) As DataTable
        For Each c As DataColumn In dt.Columns
            If c.ColumnName = "PONo" Or c.ColumnName = "AffiliateID" Or c.ColumnName = "SupplierID" Or c.ColumnName = "PartNo" Or c.ColumnName = "PartName" Or c.ColumnName = "SupplierSuratJalanNo" Then
