...
#Region "FORM EVENTS"
    ...
    Private Sub grid_CellEditorInitialize(ByVal sender As Object, ByVal e As DevExpress.Web.ASPxGridView.ASPxGridViewEditorEventArgs) Handles grid.CellEditorInitialize
        If (e.Column.FieldName = "NoUrut" Or e.Column.FieldName = "AffiliateID" Or e.Column.FieldName = "BuyerCode" _
            Or e.Column.FieldName = "BuyerName" Or e.Column.FieldName = "Address" Or e.Column.FieldName = "City" Or e.Column.FieldName = "PostalCode" _
            Or e.Column.FieldName = "Phone1" Or e.Column.FieldName = "Phone2" Or e.Column.FieldName = "Fax" Or e.Column.FieldName = "NPWP" _
            Or e.Column.FieldName = "DefaultCls") _
...
#Region "PROCEDURE"
    Private Sub bindData()
        ...
        Using sqlConn As New SqlConnection(clsGlobal.ConnectionString)
            ...
            ls_SQL = " select  " & vbCrLf & _
                  "  	row_number() over (order by AffiliateID, BuyerCode) NoUrut,  " & vbCrLf & _
                  "  	RTRIM(AffiliateID)AffiliateID,  " & vbCrLf & _
                  "  	RTRIM(BuyerCode)DeliveryLocationCode,  " & vbCrLf & _
                  "  	RTRIM(BuyerName)DeliveryLocationName,  " & vbCrLf & _
    ...
    Private Sub up_GridLoadWhenEventChange()
        ...
        Using sqlConn As New SqlConnection(clsGlobal.ConnectionString)
            ...
            ls_SQL = " select top 0  '' NoUrut, ''AffiliateID, '' BuyerCode, '' BuyerName, '' Address, '' City, '' PostalCode, '' Phone1, '' Phone2, '' Fax, '' NPWP, '' PODeliveryBy, '' DefaultCls "
    ...
    Private Function AlreadyUsed(ByVal pAffiliateID As String, ByVal pDeliveryLoc As String) As Boolean
        Try
            ...
            Using sqlConn As New SqlConnection(clsGlobal.ConnectionString)
                ...
                ls_SQL = " SELECT AffiliateID FROM MS_Buyer WHERE AffiliateID = '" & Trim(pAffiliateID) & "' and BuyerCode= '" & Trim(pDeliveryLoc) & "'"
        ...
        Try
            ...
            Using sqlConn As New SqlConnection(clsGlobal.ConnectionString)
                ...
                Using sqlTran As SqlTransaction = sqlConn.BeginTransaction("DeliveryPlace")
                    ls_SQL = " DELETE MS_Buyer " & vbCrLf & _
                                " WHERE AffiliateID = '" & Trim(pAffiliateID) & "' and BuyerCode= '" & Trim(pDeliveryLoc) & "'" & vbCrLf
    ...
    Private Sub SaveData(ByVal pIsNewData As Boolean, _
        ...
        Try
            Using sqlConn As New SqlConnection(clsGlobal.ConnectionString)
                ...
                ls_SQL = " SELECT AffiliateID FROM MS_Buyer WHERE AffiliateID ='" & pAffiliateID & "' and BuyerCode ='" & pDeliveryLoc & "'  "
                Dim sqlDA As New SqlDataAdapter(ls_SQL, sqlConn)
                Dim ds As New DataSet
                sqlDA.Fill(ds)
        ...
        Using sqlConn As New SqlConnection(clsGlobal.ConnectionString)
            ...
            Using sqlTran As SqlTransaction = sqlConn.BeginTransaction("DeliveryPlace")
                ...
                If pIsNewData = True Then
                    ...
                    '    ls_SQL = " Update MS_Buyer set DefaultCls = 0 where affiliateID = '" & cboAffiliateCode.Text & "'" & vbCrLf
                    'End If

                    ls_SQL = ls_SQL + " INSERT INTO MS_Buyer " & _
                                "(AffiliateID, BuyerCode, BuyerName, Address, City, PostalCode, Phone1, Phone2, Fax, NPWP, DefaultCls, EntryDate, EntryUser, UpdateDate, UpdateUser)" & _
                                " VALUES ('" & pAffiliateID & "','" & pDeliveryLoc & "','" & pDeliveryLocName & "','" & pAddress & "', " & vbCrLf & _
                                " '" & pCity & "','" & pPostalCode & "','" & pPhone1 & "','" & pPhone2 & "', " & vbCrLf & _
                                " '" & pFax & "','" & pNPWP & "','" & pDefaultCls & "', " & vbCrLf & _
                ...
                ElseIf pIsNewData = False Then
                    ...
                    ls_SQL = ls_SQL + " UPDATE MS_Buyer SET " & _
                            "BuyerName = '" & pDeliveryLocName & "', " & vbCrLf & _
                    ...
                    " WHERE AffiliateID ='" & pAffiliateID & "' and BuyerCode ='" & pDeliveryLoc & "' " & vbCrLf
                    ls_MsgID = "1002"
    ...
    Private Sub epplusExportExcel(ByVal pFilename As String, ByVal pSheetName As String,
        ...
        Try
            Dim tempFile As String = "Buyer Master " & Format(Now, "yyyyMMdd hhmmss") & ".xlsx"
