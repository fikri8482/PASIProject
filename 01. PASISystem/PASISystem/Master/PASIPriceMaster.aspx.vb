...
#Region "FORM EVENTS"
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If (Not IsPostBack) AndAlso (Not IsCallback) Then
            Session("MenuDesc") = "SELLING PRICE MASTER"
            up_FillCombo()
            DeleteHistory()
            dt1.Value = Now
            dt2.Value = Now
    ...
    Private Sub grid_CellEditorInitialize(ByVal sender As Object, ByVal e As DevExpress.Web.ASPxGridView.ASPxGridViewEditorEventArgs) Handles grid.CellEditorInitialize
        If (e.Column.FieldName = "NoUrut" Or e.Column.FieldName = "PartNo" Or e.Column.FieldName = "PartName" _
            Or e.Column.FieldName = "AffiliateID" Or e.Column.FieldName = "AffiliateName" Or e.Column.FieldName = "StartDate" _
    ...
    Private Sub btnUpload_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnUpload.Click
        Response.Redirect("~/Upload/UploadPASIPrice.aspx")
    End Sub
            ...
            Select Case pAction
                ...
                Case "delete"
                    ...
                    If HF.Get("DeleteCls") = "0" Then
                        Call DeleteData(pAffiliateID, pPartNo, pCurrCls, pStartDate, pPackingCls)
                    Else
                        Call DeleteDataRec(pAffiliateID, pPartNo, pCurrCls, pStartDate, pPackingCls)
                    End If
                ...
                Case "downloadSummary"
                    ...
                    Dim pAdditonal As String = ""

                    If checkbox1.Checked = True Then
                        pAdditonal = pAdditonal + " AND StartDate = '" & Format(dt1.Value, "dd MMM yyyy") & "' " & vbCrLf
                    End If

                    If checkbox2.Checked = True Then
                        pAdditonal = pAdditonal + " AND EndDate = '" & Format(dt2.Value, "dd MMM yyyy") & "' " & vbCrLf
                    End If

                    If checkbox3.Checked = True Then
                        pAdditonal = pAdditonal + " AND EffectiveDate = '" & Format(dt3.Value, "dd MMM yyyy") & "' " & vbCrLf
                    End If

                    Dim dtProd As DataTable = clsMaster.GetTablePricePASI(cboPartNo.Text, cboAffiliate.Text, pAdditonal)
                    FileName = "TemplateMSPASIPrice.xlsx"
...
#Region "PROCEDURE"
    Private Sub bindData()
        ...
        Using sqlConn As New SqlConnection(clsGlobal.ConnectionString)
            ...
            ls_SQL = " SELECT row_number() over (order by PartNo, AffiliateID) NoUrut, * FROM (" & vbCrLf & _
                  " select " & vbCrLf & _
                  " 	RTRIM(a.PartNo)PartNo, " & vbCrLf & _
                  " 	RTRIM(d.PartName)PartName, " & vbCrLf & _
                  " 	RTRIM(a.AffiliateID)AffiliateID, " & vbCrLf & _
                  " 	RTRIM(c.AffiliateName)AffiliateName, " & vbCrLf & _
                  ...
                  " inner join MS_Affiliate c on a.AffiliateID = c.AffiliateID " & vbCrLf & _
                  ...
                  " left join MS_PriceCls g on a.PriceCls = g.PriceCls " & vbCrLf
    ...
    Private Sub up_GridLoadWhenEventChange()
        ...
        Using sqlConn As New SqlConnection(clsGlobal.ConnectionString)
            ...
            ls_SQL = " select top 0  '' NoUrut, ''PackingCls,''PriceCls, ''PriceDesc, ''PackingDesc,'' as PartNo, '' PartName, '' AffiliateID, '' AffiliateName, '' StartDate, '' EndDate, '' EffectiveDate, '' CurrCls, '' Price"
    ...
    Private Sub DeleteData(ByVal pAffiliateID As String, ByVal pPartNo As String, ByVal pCurrency As String, ByVal pStartDate As Date, ByVal pPackingCls As String)
            ...
            Using sqlConn As New SqlConnection(clsGlobal.ConnectionString)
                ...
                Using sqlTran As SqlTransaction = sqlConn.BeginTransaction("Price")
                    ls_SQL = " INSERT INTO MS_Price_HISTORY" & vbCrLf & _
                             " SELECT * FROM MS_Price WHERE PartNo='" & pPartNo & "' and AffiliateID ='" & pAffiliateID & "'" & vbCrLf & _
    ...
    Private Sub DeleteDataRec(ByVal pAffiliateID As String, ByVal pPartNo As String, ByVal pCurrency As String, ByVal pStartDate As Date, ByVal pPackingCls As String)
        'ByVal pSupplierID As String,
        Try
            Dim ls_SQL As String = ""
            Dim x As Integer
            ...
            Using sqlConn As New SqlConnection(clsGlobal.ConnectionString)
                ...
                Using sqlTran As SqlTransaction = sqlConn.BeginTransaction("Price")
                    ls_SQL = " INSERT INTO MS_Price" & vbCrLf & _
                             " SELECT * FROM MS_Price_HISTORY WHERE PartNo='" & pPartNo & "' and AffiliateID ='" & pAffiliateID & "'" & vbCrLf & _
                             " and CurrCls='" & pCurrency & "' and StartDate ='" & pStartDate & "' and PackingCls = '" & pPackingCls & "'" & vbCrLf
                    Dim SqlComm As New SqlCommand(ls_SQL, sqlConn, sqlTran)
                    x = SqlComm.ExecuteNonQuery()
    ...
    Private Sub SaveData(ByVal pIsNewData As Boolean, _
                         ...
                         Optional ByVal pPriceCls As String = "")
        ...
        Try
            Using sqlConn As New SqlConnection(clsGlobal.ConnectionString)
                ...
                ls_SQL = " SELECT PartNo, PackingCls, EffectiveDate, EndDate, Price, PriceCls, PackingCls  FROM MS_Price WHERE PartNo='" & pPartNo & "' and AffiliateID = '" & pAffiliateID & "' and StartDate ='" & pStartDate & "' and CurrCls='" & pCurrency & "' and PackingCls in ('" & pPackingCls & "','')"
                ...
                If ds.Tables(0).Rows.Count > 0 Then
                    pIsNewData = False
                    If Trim(ds.Tables(0).Rows(0)("PackingCls")) = "" Then
                        ls_Sql2 = "UPDATE MS_Price SET " & vbCrLf & _
                           ...
                           "WHERE PartNo='" & pPartNo & "' and AffiliateID ='" & pAffiliateID & "'" & vbCrLf & _
                           " and StartDate ='" & pStartDate & "' and CurrCls='" & pCurrency & "' and PackingCls = ''" & vbCrLf
                    Else
                        ls_Sql2 = "UPDATE MS_Price SET " & vbCrLf & _
                            "EndDate='" & pEndDate & "'," & vbCrLf & _
                            "EffectiveDate='" & pEffectiveDate & "'," & vbCrLf & _
            ...
            Using sqlTran As SqlTransaction = sqlConn.BeginTransaction("Price")
                ...
                If pIsNewData = True Then
                    ...
                    ls_SQL = " INSERT INTO MS_Price " & _
                                "(PartNo, AffiliateID, StartDate, EndDate, EffectiveDate, EntryDate, EntryUser, CurrCls, Price, PackingCls, PriceCls)" & _
                                " VALUES ('" & pPartNo & "','" & pAffiliateID & "','" & pStartDate & "','" & pEndDate & "','" & pEffectiveDate & "'," & vbCrLf & _
                                " GETDATE(),'" & Session("UserID").ToString & "','" & pCurrency & "','" & pPrice & "','" & pPackingCls & "','" & pPriceCls & "')" & vbCrLf
                    ls_MsgID = "1001"
    ...
    Private Sub epplusExportExcel(ByVal pFilename As String, ByVal pSheetName As String,
        ...
        Try
            Dim tempFile As String = "PASI Price Master " & Format(Now, "yyyyMMdd hhmmss") & ".xlsx"
            ...
            With ws
                ...
                Dim rgAll As ExcelRange = .Cells(2, 1, irow + 2, 9)
