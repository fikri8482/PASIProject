    ...
    Private Sub bindData()
        ...
        Using sqlConn As New SqlConnection(clsGlobal.ConnectionString)
            ...
            ls_SQL = " select " & vbCrLf & _
                  " 	row_number() over (order by a.AffiliateID, a.DockID) NoUrut, " & vbCrLf & _
                  " 	RTRIM(a.AffiliateID)AffiliateID, " & vbCrLf & _
                  " 	RTRIM(b.AffiliateName)AffiliateName, " & vbCrLf & _
                  " 	RTRIM(a.DockID)DockID " & vbCrLf & _
                  " from MS_Dock a " & vbCrLf & _
    ...
    Private Function AlreadyUsed(ByVal pAffiliate As String, ByVal pDockID As String) As Boolean
        Try
            ...
            Using sqlConn As New SqlConnection(clsGlobal.ConnectionString)
                ...
                ls_SQL = " SELECT PartNo FROM Kanban_Barcode WHERE DockID = '" & Trim(pDockID) & "' and AffiliateID = '" & Trim(pAffiliate) & "'"
        ...
        Try
            ...
            Using sqlConn As New SqlConnection(clsGlobal.ConnectionString)
                ...
                Using sqlTran As SqlTransaction = sqlConn.BeginTransaction("AffiliateMaster")
                    ls_SQL = " DELETE MS_Dock " & vbCrLf & _
                                " WHERE AffiliateID='" & pAffiliateID & "' and DockID ='" & pDockID & "' "
    ...
    Private Sub SaveData(ByVal pIsNewData As Boolean, _
        ...
        Try
            Using sqlConn As New SqlConnection(clsGlobal.ConnectionString)
                ...
                ls_SQL = " SELECT DOCKID FROM MS_Dock WHERE AffiliateID ='" & pAffiliateID & "' and DockID = '" & IIf(ls_Supp = "", pDockID, ls_Supp) & "'"
        ...
        Using sqlConn As New SqlConnection(clsGlobal.ConnectionString)
            ...
            Using sqlTran As SqlTransaction = sqlConn.BeginTransaction("CostCenter")
                ...
                If pIsNewData = True Then
                    '#INSERT NEW DATA
                    ls_SQL = " INSERT INTO MS_Dock " & _
                                "(AffiliateID, DockID, EntryDate, EntryUser)" & _
                ...
                ElseIf pIsNewData = False Then
                    '#UPDATE DATA
                    ls_SQL = "UPDATE MS_Dock SET " & _
                            "DockID ='" & pDockID & "'," & _
                            "UpdateDate = getdate()," & _
                            "UpdateUser ='" & admin & "'" & _
                            "WHERE AffiliateID ='" & pAffiliateID & "' and DockID = '" & ls_Supp & "'"
                    ls_MsgID = "1002"
