...
Public Class ETDSupplierMaster
#Region "DECLARATION"
...
#Region "FORM EVENTS"
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If (Not IsPostBack) AndAlso (Not IsCallback) Then
            Session("MenuDesc") = "E.T.D SUPPLIER VIA PASI"
            up_FillCombo()
            DtPeriod.Focus()
            DtPeriod.Value = Now
    ...
    Private Sub grid_CellEditorInitialize(ByVal sender As Object, ByVal e As DevExpress.Web.ASPxGridView.ASPxGridViewEditorEventArgs) Handles grid.CellEditorInitialize
        If (e.Column.FieldName = "NoUrut" Or e.Column.FieldName = "ETAPASI" Or e.Column.FieldName = "ETDSupplier" _
    ...
    Private Sub btnUpload_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnUpload.Click
        Response.Redirect("~/Upload/UploadETDViaPASI.aspx")
    End Sub
    ...
    Private Sub grid_CustomCallback(ByVal sender As Object, ByVal e As DevExpress.Web.ASPxGridView.ASPxGridViewCustomCallbackEventArgs) Handles grid.CustomCallback
        Try
            ...
            Select Case pAction
                Case "save"
                    Dim pSupplierID As String = Split(e.Parameters, "|")(2)
                    Dim lb_IsUpdate As Boolean = ValidasiInput(pSupplierID)

                    Dim Supplier As String = Split(e.Parameters, "|")(2)
                    Dim ETAPASI As String = Split(e.Parameters, "|")(3)
                    Dim ETDSupplier As String = Split(e.Parameters, "|")(4)
                ...
                Case "delete"
                    ...
                    'Dim ls_date As String = ""
                    'ls_date = Mid(pStartDate, 5, 11)
                    If AlreadyUsed(pSupplierID, pStartDate) = False Then
                        'pSupplierID,
                        Call DeleteData(pSupplierID, pStartDate)
                        Call bindData()
                    End If
                    txtMode.Text = "new"
                ...
                Case "downloadSummary"
                    Dim psERR As String = ""
                    Dim dtProd As DataTable = clsMaster.GetTableETDViaPASI(DtPeriod.Value, cboSupplier.Text)
                    FileName = "TemplateMSETDSupplierViaPASI.xlsx"
...
#Region "PROCEDURE"
    Private Sub bindData()
        ...
        If Format(DtPeriod.Value, "yyyy-MM-dd") <> "" Then
            pWhere = pWhere + " and ETAPASI like '%" & Format(DtPeriod.Value, "yyyy-MM-dd") & "%' "
        End If
        ...
        Using sqlConn As New SqlConnection(clsGlobal.ConnectionString)
            ...
            ls_SQL = " DECLARE @Period AS DATETIME  " & vbCrLf & _
                  ...
                  " ETAPASI = COALESCE(ETAPASI, " & vbCrLf & _
            ...
            ls_SQL = ls_SQL + " * " & vbCrLf & _
                              ...
                              " ) DS " & vbCrLf & _
                               " LEFT JOIN MS_ETD_SUPPLIER_PASI MEP ON DAY(MEP.ETAPASI) = DS.SeqNo " & vbCrLf & _
                              " AND CONVERT(CHAR(6), ETAPASI, 112) = LEFT(CONVERT(CHAR(8), @Period, 112),6) " & vbCrLf & _
                              " LEFT JOIN MS_Supplier MS ON MS.SupplierID = MEP.SupplierID " & vbCrLf & _
                              " WHERE DS.SeqNo BETWEEN 1 AND 31 " & vbCrLf & _
            ...
            ls_SQL = ls_SQL + "  FROM ( SELECT TOP 31  *  " & vbCrLf & _
                              ...
                              " 		 LEFT JOIN  " & vbCrLf & _
                              " 		 (SELECT *  " & vbCrLf & _
                              " 		    FROM MS_ETD_SUPPLIER_PASI  " & vbCrLf & _
                              " 		  WHERE SupplierID = '" & cboSupplier.Text.Trim & "' " & vbCrLf & _
                              " 		 ) MEP " & vbCrLf & _
                              " 		 ON DAY(MEP.ETAPASI) = DS.SeqNo  " & vbCrLf & _
                              " 		 AND CONVERT(CHAR(6), ETAPASI, 112) = LEFT(CONVERT(CHAR(8), @Period, 112),6)  "

            ls_SQL = ls_SQL + " 		  LEFT JOIN MS_Supplier MS ON MS.SupplierID = MEP.SupplierID  " & vbCrLf & _
    ...
    Private Function AlreadyUsed(ByVal pSupplierID As String, ByVal pStartDate As Date) As Boolean
        Try
            Dim ls_SQL As String = ""
            ...
            Using sqlConn As New SqlConnection(clsGlobal.ConnectionString)
                ...
                ls_SQL = " SELECT * FROM MS_ETD_SUPPLIER_PASI WHERE SupplierID ='" & pSupplierID.Trim & "' and ETAPASI='" & pStartDate & "' "
    ...
    Private Sub DeleteData(ByVal pSupplierID As String, ByVal pStartDate As Date)
        ...
        Try
            ...
            Using sqlConn As New SqlConnection(clsGlobal.ConnectionString)
                ...
                Using sqlTran As SqlTransaction = sqlConn.BeginTransaction("DelETDSupplierMaster")
                    ls_SQL = " DELETE MS_ETD_SUPPLIER_PASI " & vbCrLf & _
                                " WHERE SupplierID ='" & pSupplierID.Trim & "'" & vbCrLf & _
                                " and ETAPASI ='" & pStartDate & "'" & vbCrLf
        ...
        Using sqlConn As New SqlConnection(clsGlobal.ConnectionString)
            ...
            Using sqlTran As SqlTransaction = sqlConn.BeginTransaction("Price")
                ...
                If pIsNewData = True Then
                    '#INSERT NEW DATA
                    ls_SQL = " INSERT INTO MS_ETD_SUPPLIER_PASI " & _
                                "(SupplierID, ETAPASI, ETDSupplier)" & _
                                " VALUES ('" & pSupplierID.Trim & "','" & pStartDate & "','" & pEndDate & "' )" & vbCrLf
                    ls_MsgID = "1001"
                ...
                ElseIf pIsNewData = False Then
                    ...
                    ls_SQL = "UPDATE MS_ETD_SUPPLIER_PASI SET " & _
    ...
    Private Sub epplusExportExcel(ByVal pFilename As String, ByVal pSheetName As String,
        ...
        Try
            Dim tempFile As String = "ETD Supplier Master " & Format(Now, "yyyyMMdd hhmmss") & ".xlsx"
