...
#Region "PROCEDURE"
    ...
    Private Sub GridLoadExcel()
        ...
        ls_SQL = " SELECT  ColNo = CONVERT(Numeric, ROW_NUMBER() OVER (ORDER BY POM.AffiliateID)), AffiliateID = POM.AffiliateID , " & vbCrLf & _
              ...
              "         cls = 'BY PASI' , " & vbCrLf & _
              "         OrderNo = COALESCE(POM.OrderNo, PRM.OrderNo) , " & vbCrLf & _
              "         FirmQty = '' , " & vbCrLf & _
              "         Curr = ISNULL(MC.DESCRIPTION,'') , " & vbCrLf & _
              "         Price = ISNULL(ID.Price,0) , "

        ls_SQL = ls_SQL + "         Amount = ISNULL(ID.Amount, 0) , " & vbCrLf & _
                          "         Qty = CASE COALESCE(POM.Week, PRM.Week)	WHEN  '1' THEN COALESCE(POD.Week1, PRD.Week1) " & vbCrLf & _
                          " 												WHEN  '2' THEN COALESCE(POD.Week2, PRD.Week2) " & vbCrLf & _
                          " 												WHEN  '3' THEN COALESCE(POD.Week3, PRD.Week3) " & vbCrLf & _
                          " 												WHEN  '4' THEN COALESCE(POD.Week4, PRD.Week4) " & vbCrLf & _
                          " 												WHEN  '5' THEN COALESCE(POD.Week5, PRD.Week5) END " & vbCrLf & _
        ...
        ls_SQL = ls_SQL + "                     UNION ALL " & vbCrLf & _
                          ...
                          "                     FROM    PoRev_Master_Export " & vbCrLf & _
                          "                     ORDER BY PORevNo " & vbCrLf & _
                          "                   ) PRM ON PRM.PONO = POD.PONO "

        ls_SQL = ls_SQL + "                            AND PRM.AffiliateID = POD.AffiliateID " & vbCrLf & _
                          ...
                          "                                                 AND POM.OrderNo = RD.OrderNo "

        ls_SQL = ls_SQL + "                                                 AND POD.PartNo = RD.PartNo " & vbCrLf & _
                          ...
                          "                                                       AND ID.OrderNo = IM.OrderNo "

        ls_SQL = ls_SQL + "                                                       AND ID.PartNo = POD.PartNo  " & vbCrLf & _
        ...
        ls_SQL = ls_SQL + " ORDER BY ColNo, POM.PONO, POD.PartNo "
        ...
        Try
            ...
            For irow = 0 To pData.Rows.Count - 1
                If pData.Rows.Count > 0 Then
                    ...
                    ws.Cells("N" & iRowTmp).Value = pData.Rows(irow)("Qty")
