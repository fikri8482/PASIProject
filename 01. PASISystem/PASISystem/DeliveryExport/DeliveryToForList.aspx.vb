...
Public Class DeliveryToForList
...
#Region "PROCEDURE"
    Private Sub up_fillcombo()
        ...
        ls_sql = ""
        'AFFILIATE
        ls_sql = "SELECT distinct AffiliateID = '" & clsGlobal.gs_All & "', AffiliateName = '" & clsGlobal.gs_All & "' from MS_Affiliate where isnull(overseascls, '0') = '1' " & vbCrLf & _
        ...
        Using sqlConn As New SqlConnection(clsGlobal.ConnectionString)
            ...
            ls_sql = "SELECT distinct ForwarderID = '" & clsGlobal.gs_All & "', ForwarderName = '" & clsGlobal.gs_All & "' from MS_Forwarder " & vbCrLf & _
                "Union all SELECT RTRIM(ForwarderID)ForwarderID,RTRIM(ForwarderName)ForwarderName FROM dbo.MS_Forwarder " & vbCrLf
            sqlConn.Open()

            Dim sqlDAAA As New SqlDataAdapter(ls_sql, sqlConn)
            ...
            With cboForwarder
                ...
                .Columns(1).Width = 240
                .SelectedIndex = 0
                txtForwarder.Text = clsGlobal.gs_All
    ...
    Private Sub up_GridLoad()
        ...
        Using sqlConn As New SqlConnection(clsGlobal.ConnectionString)
            ...
            If checkbox1.Checked = True Then
                ls_Filter = ls_Filter + " AND SDM.DeliveryDate <= '" & Format(dt1.Value, "yyyy-MM-dd") & "' " & vbCrLf
            End If
            ...
            If checkbox2.Checked = True Then
                ls_Filter = ls_Filter + " AND Convert(char(10),Convert(datetime,PDM.DeliveryDate),120) between '" & Format(dtfrom.Value, "yyyy-MM-dd") & "' and '" & Format(dtto.Value, "yyyy-MM-dd") & "'" & vbCrLf
            End If

            If cboaffiliate.Text <> clsGlobal.gs_All And cboaffiliate.Text <> "" Then
                ls_Filter = ls_Filter + " AND SDP.AffiliateID = '" & Trim(cboaffiliate.Text) & "'" & vbCrLf
            End If

            If cbopart.Text <> clsGlobal.gs_All And cbopart.Text <> "" Then
                ls_Filter = ls_Filter + "AND SDP.PartNo = '" & Trim(cbopart.Text) & "'" & vbCrLf
            End If

            If txtpono.Text <> "" Then
                ls_Filter = ls_Filter + "and SDP.OrderNo LIKE '%" & txtpono.Text & "%'" & vbCrLf
            End If
            ...
            ls_SQL = "   SELECT   " & vbCrLf & _
                  "   	Act, coldetail = coldetail + '|' + coldetailname, coldetailname , colno =CONVERT(char,ROW_NUMBER() OVER(ORDER BY HpasiSJ,HSupSJ,h_poorder,h_idxorder , urut)) ,      " & vbCrLf & _
                  "   	colaffiliatecode, colaffiliatename, colpono,     " & vbCrLf & _
                  "   	colsuppliercode, colsuppliername,colPalet, coldeldate, colsj,    " & vbCrLf & _
                  ...
                  "   	colremaining, colpasideliveryqty, coldeliverydate, coldeliveryby, H_POORDER, H_IDXORDER, H_AFFILIATEORDER,    " & vbCrLf & _
                  "   	HSupSJ, HPasiSJ,urut  FROM (   " & vbCrLf & _
                  "   	SELECT DISTINCT *   " & vbCrLf & _
                  "   		 FROM (     " & vbCrLf & _
                  "   		 --DATA DOPASI   " & vbCrLf & _
                  "   		 SELECT DISTINCT      				  Act = 0 ,     "

            ls_SQL = ls_SQL + "   				  coldetail = 'DeliveryToForEntry.aspx?prm='     " & vbCrLf & _
                              "   				  + CONVERT(CHAR(12), CONVERT(DATETIME, ISNULL(PDM.DeliveryDate, '')), 106)     " & vbCrLf & _
                              "   				  + '|' + RTRIM(SDP.AffiliateID) + '|' + RTRIM(MA.AffiliateName) + '|'     " & vbCrLf & _
                              "   				  + Rtrim(REPLACE(ISNULL(PDM.SuratJalanNo,''),'&','DAN')) + '|'     " & vbCrLf & _
                              "   				  + RTRIM(ISNULL(SDP.ForwarderID, '')) + '|'     " & vbCrLf & _
                              "   				  + RTRIM(ISNULL(MF.ForwarderName, '')) + '|'     " & vbCrLf & _
                              ...
                              " 				  + RTRIM(ISNULL(SDP.OrderNo, '')) + '''|'       "

            ls_SQL = ls_SQL + "   				  + RTRIM(ISNULL(SDP.PalletNo, '')) + '|'     " & vbCrLf & _
                              "   				  + RTRIM(ISNULL(SDP.SupplierID, '')) + '|'     " & vbCrLf & _
                              "   				  + RTRIM(ISNULL(MS.SupplierName, '')) + '|'     " & vbCrLf & _
                              "   				  + Rtrim(REPLACE(ISNULL(SDP.SuratJalanNo,''),'&','DAN'))  + '|'     " & vbCrLf & _
                              ...
                              "   									   ELSE 'DETAIL' END ,             " & vbCrLf & _
                              " 				  colno = '' ,     "

            ls_SQL = ls_SQL + "   				  colaffiliatecode = SDP.AffiliateID ,     " & vbCrLf & _
                              "   				  colaffiliatename = MA.AffiliateName ,      " & vbCrLf & _
                              "   				  colpono = SDP.OrderNo ,     " & vbCrLf & _
                              "   				  colsuppliercode = SDP.SupplierID ,     " & vbCrLf & _
                              "   				  colsuppliername = MS.SupplierName ,      " & vbCrLf & _
                              "                   colPalet = SDP.PalletNo, " & vbCrLf & _
                              ...
                              "   				  colpartno = ISNULL(PDD.PartNo, '') , colpartname = ISNULL(MP.PartName,'') ,     "

            ls_SQL = ls_SQL + "   				  coluom = '' ,     " & vbCrLf & _
                              ...
                              "   				  SDP.ShippingInstructionNo H_POORDER ,               " & vbCrLf & _
                              "   				  H_IDXORDER = 0 ,      				    " & vbCrLf & _
                              "   				  H_AFFILIATEORDER = SDP.AffiliateID ,      "

            ls_SQL = ls_SQL + "   				  HSupSJ = SDM.SuratJalanNo,    " & vbCrLf & _
                              "   				  HPasiSJ = isnull(PDD.SuratJalanNo,''), urut = 0  " & vbCrLf & _
                              "   		          FROM    dbo.ShippingInstruction_DetailPallet SDP  				  " & vbCrLf & _
                              "   				  LEFT JOIN dbo.DOSupplier_Detail_Export SDD ON SDP.AffiliateID = SDD.AffiliateID       " & vbCrLf & _
                              "   														 AND SDP.OrderNo = SDD.PONo " & vbCrLf & _
                              " 														 AND SDD.SuratJalanNo = SDP.SuratJalanNo     " & vbCrLf & _
                              "   														 AND SDP.SupplierID = SDD.SupplierID     " & vbCrLf & _
                              "   				  LEFT JOIN dbo.DOSupplier_Master_Export SDM ON SDM.AffiliateID = SDD.AffiliateID                                                              " & vbCrLf & _
                              " 														 AND SDM.SuratJalanNo = SDD.SuratJalanNo     "

            ls_SQL = ls_SQL + "   														 AND SDM.SupplierID = SDD.SupplierID     " & vbCrLf & _
                              "   				  LEFT JOIN dbo.ReceiveForwarder_Detail PRD ON SDP.AffiliateID = PRD.AffiliateID       " & vbCrLf & _
                              "   														  AND SDP.SupplierID = PRD.SupplierID     "

            ls_SQL = ls_SQL + "   														  AND SDP.PartNo = PRD.PartNo     " & vbCrLf & _
                              "   														  AND SDP.OrderNo = PRD.PONo     " & vbCrLf & _
                              ...
                              " 				  LEFT JOIN dbo.DOPASI_Detail_Export PDD ON PRD.AffiliateID = PDD.AffiliateID   " & vbCrLf & _
                              "   														  AND PRD.SupplierID = PDD.SupplierID     " & vbCrLf & _
                              "   														  AND PRD.PartNo = PDD.PartNo     " & vbCrLf & _
                              "   														  AND PRD.PoNo = PDD.orderno "

            ls_SQL = ls_SQL + "   				  LEFT JOIN dbo.DOPASI_Master_Export PDM ON PDD.AffiliateID = PDM.AffiliateID     " & vbCrLf & _
                              "   													 AND PDD.SuratJalanNo = PDM.SuratJalanNo     " & vbCrLf & _
                              "                                                      AND PDD.SupplierID = PDM.SupplierID  " & vbCrLf & _
                              "   				  LEFT JOIN dbo.MS_Parts MP ON MP.PartNo = SDP.PartNo     " & vbCrLf & _
                              "   				  LEFT JOIN dbo.MS_Affiliate MA ON MA.AffiliateID = SDP.AffiliateID     " & vbCrLf & _
                              "                   LEFT JOIN dbo.MS_Forwarder MF ON Mf.ForwarderID = SDP.ForwarderID  " & vbCrLf & _
                              "   				  LEFT JOIN dbo.MS_Supplier MS ON MS.SupplierID = SDP.SupplierID     " & vbCrLf & _
                              "   		WHERE   'A' = 'A'   " & vbCrLf & _
                              "   		AND ISNULL(PDD.SuratJalanNo,'') <> ''  " & vbCrLf & _
                              "  "
            ...
            ls_SQL = ls_SQL + " UNION ALL   " & vbCrLf & _
                     "   		 SELECT DISTINCT      				  Act = 0 ,     " & vbCrLf & _
                     "   				  coldetail = 'DeliveryToForEntry.aspx?prm='     " & vbCrLf & _
                     "   				  + CONVERT(CHAR(12), CONVERT(DATETIME, ISNULL(PDM.DeliveryDate, '')), 106)     " & vbCrLf & _
                     "   				  + '|' + RTRIM(SDP.AffiliateID) + '|' + RTRIM(MA.AffiliateName) + '|'     " & vbCrLf & _
                     "   				  + Rtrim(REPLACE(ISNULL(PDM.SuratJalanNo,''),'&','DAN')) + '|'     " & vbCrLf & _
                     "   				  + RTRIM(ISNULL(SDP.ForwarderID, '')) + '|'     " & vbCrLf & _
                     ...
                     "   				  + RTRIM(ISNULL(PDM.DriverName, '')) + '|'     " & vbCrLf & _
                     "   				  + RTRIM(ISNULL(PDM.DriverContact, '')) + '|'    " & vbCrLf & _
                     "   				  + RTRIM(ISNULL(PDM.NoPol,'')) + '|'               "

            ls_SQL = ls_SQL + "   				  + RTRIM(ISNULL(PDM.JenisArmada, '')) + '|'''  " & vbCrLf & _
                              " 				  + RTRIM(ISNULL(SDP.OrderNo, '')) + '''|'       " & vbCrLf & _
                              "   				  + RTRIM(ISNULL(SDP.PalletNo, '')) + '|'     " & vbCrLf & _
                              "   				  + RTRIM(ISNULL(SDP.SupplierID, '')) + '|'     " & vbCrLf & _
                              "   				  + RTRIM(ISNULL(MS.SupplierName, '')) + '|'     " & vbCrLf & _
                              "   				  + Rtrim(REPLACE(ISNULL(SDP.SuratJalanNo,''),'&','DAN'))  + '|'     " & vbCrLf & _
                              "   				  + Rtrim(REPLACE(ISNULL(PDD.SuratJalanNo,''),'&','DAN'))  + '|'     " & vbCrLf & _
            ...
            ls_SQL = ls_SQL + "   									   ELSE 'DETAIL' END ,             " & vbCrLf & _
                              " 				  colno = '' ,     " & vbCrLf & _
                              "   				  colaffiliatecode = SDP.AffiliateID ,     " & vbCrLf & _
                              "   				  colaffiliatename = MA.AffiliateName ,      " & vbCrLf & _
                              "   				  colpono = SDP.OrderNo ,     " & vbCrLf & _
                              "   				  colsuppliercode = SDP.SupplierID ,     " & vbCrLf & _
                              "   				  colsuppliername = MS.SupplierName ,      " & vbCrLf & _
                              ...
                              "   		              " & vbCrLf & _
                              "   				  colsj = ISNULL(SDM.SuratJalanNo, '') ,     " & vbCrLf & _
                              "   				  colpasideliverydate = CASE WHEN ISNULL(PDM.DeliveryDate,'') = '' THEN '' ELSE  CONVERT(CHAR(12), CONVERT(DATETIME, ISNULL(PDM.DeliveryDate, '')), 106) END ,     "

            ls_SQL = ls_SQL + "   				  colpasisj = PDM.SuratJalanNo ,               " & vbCrLf & _
                              "   				  colpartno = ISNULL(PDD.PartNo, '') ,  colpartname = ISNULL(MP.PartName,'') ,     " & vbCrLf & _
                              "   				  coluom = '' ,     " & vbCrLf & _
                              "   				  coldeliveryqty = '' ,     " & vbCrLf & _
                              "   				  colreceiveqty = '' ,     " & vbCrLf & _
                              "   				  coldefect = '' ,     " & vbCrLf & _
                              ...
                              "   				  coldeliverydate = CASE WHEN ISNULL(PDM.DeliveryDate,'') = '' THEN '' ELSE  CONVERT(CHAR(12), CONVERT(DATETIME, ISNULL(PDM.DeliveryDate, '')), 106) END,     " & vbCrLf & _
                              "   				  coldeliveryby = ISNULL(PDM.EntryUser, '') ,     " & vbCrLf & _
                              "   				  SDP.ShippingInstructionNo H_POORDER ,               "

            ls_SQL = ls_SQL + "   				  H_IDXORDER = 0 ,      				    " & vbCrLf & _
                              "   				  H_AFFILIATEORDER = SDP.AffiliateID ,      " & vbCrLf & _
            ...
            ls_SQL = ls_SQL + "   														 AND SDM.SupplierID = SDD.SupplierID     " & vbCrLf & _
                              "   				  left JOIN dbo.ReceiveForwarder_Detail PRD ON SDP.AffiliateID = PRD.AffiliateID       " & vbCrLf & _
                              "   														  AND SDP.SupplierID = PRD.SupplierID     " & vbCrLf & _
                              "   														  AND SDP.PartNo = PRD.PartNo     " & vbCrLf & _
                              "   														  AND SDP.OrderNo = PRD.PONo     " & vbCrLf & _
                              "   														  AND SDM.SuratJalanNo = PRD.SuratJalanNo    " & vbCrLf & _
                              "   				  LEFT JOIN dbo.ReceiveForwarder_Master PRM ON PRM.AffiliateID = PRD.AffiliateID     " & vbCrLf & _
                              "   														  AND PRM.SuratJalanNo = PRD.SuratJalanNo     " & vbCrLf & _
            ...
            ls_SQL = ls_SQL + "   													 AND PRD.PartNo = PDD.PartNo     " & vbCrLf & _
                              " 													 AND PRD.PoNo = PDD.OrderNo     " & vbCrLf & _
                              "   				  LEFT JOIN dbo.DOPASI_Master_Export PDM ON PDD.AffiliateID = PDM.AffiliateID     " & vbCrLf & _
                              "   													 AND PDD.SuratJalanNo = PDM.SuratJalanNo   " & vbCrLf & _
                              "                                                       AND PDD.SupplierID = PDM.SupplierID  " & vbCrLf & _
                              "   				  LEFT JOIN (SELECT AffiliateID,SupplierID,ForwarderID,PartNo, OrderNo, DOQty = Count(ISNULL(ShippingInstructionNo,0)),SuratJalanNo    " & vbCrLf & _
                              "                                      FROM dbo.DOPASI_Detail_Export " & vbCrLf & _
                              " 						GROUP BY AffiliateID,SupplierID,ForwarderID,PartNo,OrderNo,SuratJalanNo) REM   " & vbCrLf & _
                              "   						ON SDP.AffiliateID = REM.AffiliateID    " & vbCrLf & _
                              "   						AND SDP.SupplierID = REM.SupplierID "

            ls_SQL = ls_SQL + " 						AND SDP.ForwarderID = REM.ForwarderID " & vbCrLf & _
                              "   						AND SDP.PartNo = REM.PartNo    " & vbCrLf & _
                              "   						AND SDP.OrderNo = REM.OrderNo " & vbCrLf & _
            ...
            If rbPasiDelivery.Value = "YES" Then
                ls_SQL = ls_SQL + "WHERE isnull(HEADER.coldeliverydate, '') <> '' " & vbCrLf
            ElseIf rbPasiDelivery.Value = "NO" Then
                ls_SQL = ls_SQL + " WHERE isnull(HEADER.coldeliverydate, '') = '' "
            End If

            ls_SQL = ls_SQL + "   ORDER BY HpasiSJ,HSupSJ,h_poorder,h_idxorder , urut"
    ...
    Private Sub up_GridLoad_OLD1()
        ...
        Using sqlConn As New SqlConnection(clsGlobal.ConnectionString)
            ...
            ls_SQL = ls_SQL + "  				  LEFT JOIN dbo.DOPASI_For_Detail PDD ON PRD.AffiliateID = PDD.AffiliateID  " & vbCrLf & _
                              "  													 AND PRD.SupplierID = PDD.SupplierID    " & vbCrLf & _
                              "  													 AND PRD.PartNo = PDD.PartNo    " & vbCrLf & _
                              "  													 AND PRD.PoNo = PDD.PoNo    " & vbCrLf & _
                              "  													 AND SDM.SuratJalanNo = PDD.SuratJalanNoSupplier   " & vbCrLf & _
                              "  				  LEFT JOIN dbo.DOPASI_For_Master PDM ON PDD.AffiliateID = PDM.AffiliateID    " & vbCrLf & _
                              "  													 AND PDD.SuratJalanNo = PDM.SuratJalanNo    " & vbCrLf & _
                              "                                                      AND PDD.SupplierID = PDM.SupplierID " & vbCrLf & _
            ...
            ls_SQL = ls_SQL + "  														  AND KD.SupplierID = PRD.SupplierID    " & vbCrLf & _
                              ...
                              "  														  AND PRM.SuratJalanNo = PRD.SuratJalanNo    " & vbCrLf & _
                              "  														  AND PRM.SupplierID = PRD.SupplierID    " & vbCrLf & _
                              "  				  LEFT JOIN dbo.DOPASI_For_Detail PDD ON PRD.AffiliateID = PDD.AffiliateID  " & vbCrLf & _
                              "  													 AND PRD.SupplierID = PDD.SupplierID    " & vbCrLf & _
            ...
            ls_SQL = ls_SQL + "  													 AND PRD.PoNo = PDD.PoNo    " & vbCrLf & _
                              ...
                              "                                     FROM DOPASI_For_Detail GROUP BY AffiliateID,KanbanNo,SupplierID,PartNo,PONo,SuratJalanNoSupplier) REM  " & vbCrLf & _
            ...
            '                  "                                                   AND PRM.SuratJalanNo = PRD.SuratJalanNo   " & vbCrLf & _
            '                  "                                                   AND PRM.SupplierID = PRD.SupplierID   " & vbCrLf & _
            '                  "           LEFT JOIN dbo.DOPASI_For_Detail PDD ON KD.AffiliateID = PDD.AffiliateID   " & vbCrLf & _
            ...
            '                  "                                              AND SDM.SuratJalanNo = PDD.SuratJalanNoSupplier   " & vbCrLf & _
            '                  "           LEFT JOIN dbo.DOPASI_For_Master PDM ON PDD.AffiliateID = PDM.AffiliateID   "
    ...
    Private Sub up_GridLoad_OLD()
        ...
        Using sqlConn As New SqlConnection(clsGlobal.ConnectionString)
            ...
            ls_SQL = ls_SQL + "                                                 AND SDM.SuratJalanNo = SDD.SuratJalanNo  " & vbCrLf & _
                              ...
                              "                                                  AND PRM.SuratJalanNo = PRD.SuratJalanNo  " & vbCrLf & _
                              "                                                  AND PRM.SupplierID = PRD.SupplierID  " & vbCrLf & _
                              "          LEFT JOIN dbo.DOPASI_For_Detail PDD ON PRD.AffiliateID = PDD.AffiliateID  "
...
#End Region
    ...
    Private Sub grid_BatchUpdate(sender As Object, e As DevExpress.Web.Data.ASPxDataBatchUpdateEventArgs) Handles grid.BatchUpdate
        ...
        With grid
            ...
            If (e.UpdateValues(0).NewValues("Act").ToString()) = 1 Then
                ...
                Else
                    ls_Status = "DETAIL"
                End If
                'ls_DeliveryCode = e.UpdateValues(0).NewValues("coldeliverylocationcode").ToString()
                'ls_DeliveryName = e.UpdateValues(0).NewValues("coldeliverylocationname").ToString()

                ls_PO = "'" & Trim(e.UpdateValues(0).NewValues("colpono").ToString()) & "'"
                ls_Pallet = "'" & Trim(e.UpdateValues(0).NewValues("colPalet").ToString()) & "'"
            ...
            If e.UpdateValues.Count > 1 Then
                For i = 0 To e.UpdateValues.Count - 1
                    If (e.UpdateValues(i).NewValues("Act").ToString()) = 1 Then
                        ...
                        ls_Pallet = ls_Pallet + ",'" & Trim(e.UpdateValues(i).NewValues("colPalet").ToString()) & "'"
        ...
        Session("POList2") = ls_PO
        Session("PalletList") = ls_Pallet
        HF.Set("Update", "1")
            ...
            Select Case pAction
                ...
                Case "detail"
                    If Not IsNothing(Session("POList")) = True Then
                        DevExpress.Web.ASPxGridView.ASPxGridView.RedirectOnCallback("~/DeliveryExport/DeliveryToForEntry.aspx")
                    Else
