...
Public Class POExportListCancel
#Region "DECLARATION"
...
#Region "FORM EVENTS"
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Try
            ...
            If (Not IsPostBack) AndAlso (Not IsCallback) Then
                ...
                Session("MenuDesc") = "PO CANCELATION LIST"

                If Session("POEListCancel") <> "" Then
        ...
        Using sqlConn As New SqlConnection(clsGlobal.ConnectionString)
            ...
            Using sqlTran As SqlTransaction = sqlConn.BeginTransaction("PONo")
                ...
                For iLoop = 0 To a - 1
                    ...
                    sqlstring = "SELECT * FROM PO_Master_ExportCancel WHERE PONo = '" & Trim(ls_PONo) & "' AND OrderNo1 = '" & Trim(ls_OrderNo) & "' AND AffiliateID = '" & Trim(ls_AffiliateID) & "' AND SupplierID = '" & Trim(ls_SupplierID) & "' AND ForwarderID = '" & Trim(ls_ForwarderID) & "' AND ErrorStatus = 'OK'"
                    ...
                    If ls_Active = "1" Then
                        If pIsUpdate = False Then
                            ...
                            Exit Sub
                        Else
                            ls_SQL = " 	UPDATE dbo.PO_Master_ExportCancel " & vbCrLf & _
    ...
    Private Sub btnSubMenu_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnSubMenu.Click
        Session("M01Url") = "~/PurchaseOrderExport/POExportListCancel.aspx"
        Session.Remove("M01Url")
    ...
    Private Sub bindData()
        ...
        Using sqlConn As New SqlConnection(clsGlobal.ConnectionString)
            ...
            ls_SQL = "  select ROW_NUMBER() over (order by PONo, OrderNo, SupplierID, AffiliateID) as NoUrut , * " & vbCrLf & _
                  ...
                  "  	'DETAIL' DetailPage, coldetail = 'POExportEntryCancelDetail.aspx?prm='" & vbCrLf & _
                  ...
                  "  	+  RTRIM(ISNULL(a.PONo,' ')) + '|' + ISNULL(RTRIM(c.ConsigneeCode),'') + '|' + ISNULL(RTRIM(a.SplitReffPONo),'') + '|' + ISNULL(RTRIM(a.SplitStatus),'') " & vbCrLf & _
            ...
            ls_SQL = ls_SQL + "  from PO_Master_ExportCancel a  "

            'If Session("POStatus") = "1" Or Session("POStatus") = "2" Or Session("POStatus") = "4" Then
            '    ls_SQL = ls_SQL + "  inner join PO_Detail_ExportCancel b on a.PONo = b.PONo and a.AffiliateID = b.AffiliateID and a.SupplierID = b.SupplierID and a.OrderNo1 = b.OrderNo1 " & vbCrLf
            'End If

            'If Session("POStatus") = "3" Then
            '    ls_SQL = ls_SQL + " inner join PO_Detail_ExportCancel b on a.PONo = b.PONo and a.AffiliateID = b.AffiliateID and a.SupplierID = b.SupplierID and a.OrderNo1 = b.OrderNo1" & vbCrLf & _
            '                      " inner join PO_MasterUpload_ExportCancel pme on a.PONo = pme.PONo and a.AffiliateID = pme.AffiliateID and a.SupplierID = pme.SupplierID and a.ForwarderID = pme.ForwarderID and a.OrderNo1 = pme.OrderNo1" & vbCrLf
            'End If
    ...
    Private Sub bindDataSend()
        ...
        Using sqlConn As New SqlConnection(clsGlobal.ConnectionString)
            ...
            ls_SQL = "  select ROW_NUMBER() over (order by PONo, SupplierID, AffiliateID) as NoUrut , * " & vbCrLf & _
                  ...
                  "  ( " & vbCrLf & _
                  "  select  distinct  " & vbCrLf & _
                  "  	'DETAIL' DetailPage, coldetail = CASE WHEN ISNULL(EmergencyCls,'M') = 'M' THEN 'POExportEntryCancel.aspx?prm=' ELSE 'POExportEntryEmergency.aspx?prm=' END  " & vbCrLf & _
                  "  	+  ISNULL(RTRIM(a.Period),'') + '|'   " & vbCrLf & _
                  "  	+  ISNULL(RTRIM(a.AffiliateID),'') + '|' +  ISNULL(RTRIM(c.AffiliateName),'') + '|'  " & vbCrLf & _
                  "  	+  ISNULL(RTRIM(a.SupplierID),'') + '|' +  ISNULL(RTRIM(d.SupplierName),'') + '|'  " & vbCrLf & _
                  "  	+  ISNULL(RTRIM(a.ForwarderID),'') + '|' +  ISNULL(RTRIM(e.ForwarderName),'') + '|'  " & vbCrLf & _
            ...
            ls_SQL = ls_SQL + "  from PO_Master_ExportCancel a  " & vbCrLf & _
                              "  inner join PO_Detail_ExportCancel b on a.PONo = b.PONo and a.AffiliateID = b.AffiliateID and a.SupplierID = b.SupplierID  " & vbCrLf & _
                              "  left join MS_Affiliate c on c.AffiliateID = a.AffiliateID  " & vbCrLf & _
                              "  left join MS_Supplier d on a.SupplierID = d.SupplierID  " & vbCrLf & _
                              "  left join MS_Forwarder e on a.ForwarderID = e.ForwarderID  " & vbCrLf & _
                              "  --left join PO_MasterUpload_ExportCancel f on a.PONo = f.PONo and a.AffiliateID = f.AffiliateID and a.SupplierID = f.SupplierID and a.ForwarderID = f.ForwarderID  " & vbCrLf & _
