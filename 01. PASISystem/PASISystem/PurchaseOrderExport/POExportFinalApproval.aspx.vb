...
Public Class POExportFinalApproval
#Region "DECLARATION"
    ...
    Dim pub_PONo As String, pub_Ship As String, pub_Commercial As String, pub_SupplierID As String, pub_Remarks As String
...
#Region "FORM EVENTS"
        ...
        If (Not IsPostBack) AndAlso (Not IsCallback) Then
            If Session("M01Url") <> "" Then
                If (Not String.IsNullOrEmpty(Request.QueryString("id"))) Then
                    ...
                    txtPOEmergency.Text = clsPONo.POKanban(pub_PONo, Session("AffiliateID"), Session("SupplierID"))
                    ...
                    If clsPONo.Check_CreateKanban(pub_PONo, Session("AffiliateID"), Session("SupplierID")) = True Then
                        btnApprove.Enabled = False
                    Else
                        btnApprove.Enabled = True
                    End If
    ...
    Private Sub btnSubMenu_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnSubMenu.Click
        If Session("M01Url") <> "" Then
            ...
            Response.Redirect("~/PurchaseOrder/POFinalApprovalList.aspx")
        Else
            Session.Remove("M01Url")
            Session.Remove("Mode")
            Session.Remove("SupplierID")
    ...
    Private Sub grid_CustomCallback(ByVal sender As Object, ByVal e As DevExpress.Web.ASPxGridView.ASPxGridViewCustomCallbackEventArgs) Handles grid.CustomCallback
        Try
            ...
            Select Case pAction
                ...
                Case "save"
                    'cek already approve or not
                    Dim ls_sql
                    Dim ls_masuk As Boolean = False
                    Using sqlConn As New SqlConnection(clsGlobal.ConnectionString)
                        sqlConn.Open()

                        Using sqlTran As SqlTransaction = sqlConn.BeginTransaction("testingpono")

                            ls_sql = " select *  from PO_Master " & vbCrLf & _
                                      " where AffiliateID = '" & Session("AffiliateID") & "' and PONo = '" & cboPartNo.Text & "' and SupplierID = '" & Session("SupplierID") & "' and (FinalApproveUser is not null and FinalApproveDate is not null)"
                            ...
                            Dim da7 As New SqlDataAdapter(sqlCommNew)
                            ...
                            If ds7.Tables(0).Rows.Count > 0 Then
                                ...
                                sqlConn.Close()
                                ls_masuk = True
                                Call clsMsg.DisplayMessage(lblInfo, "7009", clsMessage.MsgType.ErrorMessage)
                                grid.JSProperties("cpMessage") = lblInfo.Text
                            End If
                        End Using
                    End Using

                    If ls_masuk = False Then
        ...
        With grid
            If .VisibleRowCount > 0 Then
                ...
                If e.GetValue("AffiliateName") = "BY PASI" Then
                    If e.DataColumn.FieldName = "Maker" Or e.DataColumn.FieldName = "MOQ" Or e.DataColumn.FieldName = "QtyBox" _
                        Or e.DataColumn.FieldName = "ForecastN1" Or e.DataColumn.FieldName = "ForecastN2" _
                        Or e.DataColumn.FieldName = "ForecastN3" Then
                        e.Cell.Text = ""
                    End If
...
#Region "PROCEDURE"
    ...
    Private Sub bindData()
        ...
        Using sqlConn As New SqlConnection(clsGlobal.ConnectionString)
            ...
            ls_SQL = ls_SQL + " 		select '2' NoUrutDesc, 'BY PASI' AffiliateName " & vbCrLf & _
                              ...
                              " 		inner join po_Detail b on a.PONo = b.PONo and a.SupplierID = b.SupplierID and a.AffiliateID = b.AffiliateID "

            ls_SQL = ls_SQL + " 		inner join MS_Parts c on b.PartNo = c.PartNo " & vbCrLf & _
                              " 		inner join MS_UnitCls d on d.UnitCls = c.UnitCls " & vbCrLf & _
                              " 		where a.PONo = '" & cboPartNo.Text & "' and a.AffiliateID = '" & Session("AffiliateID") & "' and a.SupplierID = '" & Session("SupplierID") & "' " & pWhereKanban & "" & vbCrLf & _
                              " 	)tb1b " & vbCrLf & _
                              " )tbl1 " & vbCrLf & _
                              " left join " & vbCrLf & _
                              " ( " & vbCrLf & _
            ...
            ls_SQL = ls_SQL + " 		ForecastN1 = isnull((select qty from MS_Forecast MF where MF.PartNo = b.PartNo and b.AffiliateID = MF.AffiliateID and YEAR(Period) = Year(DATEADD(MONTH,1,'" & Format(dtPeriodFrom.Value, "yyyy-MM-dd") & "')) and MONTH(Period) = MONTH(DATEADD(MONTH,1,'" & Format(dtPeriodFrom.Value, "yyyy-MM-dd") & "'))),0),  " & vbCrLf & _
            ...
            ls_SQL = ls_SQL + "  		0 DeliveryD6Old, 0 DeliveryD7Old, 0 DeliveryD8Old, 0 DeliveryD9Old, 0 DeliveryD10Old,  " & vbCrLf & _
                              ...
                              " 	left join MS_CurrCls e on e.CurrCls = b.CurrCls "

            ls_SQL = ls_SQL + " 	where a.PONo = '" & cboPartNo.Text & "' and a.AffiliateID = '" & Session("AffiliateID") & "' and a.SupplierID = '" & Session("SupplierID") & "' " & vbCrLf & _
                              " 	union all " & vbCrLf & _
                              " 	select  " & vbCrLf & _
            ...
            ls_SQL = ls_SQL + " 		from Affiliate_Master a " & vbCrLf & _
                              ...
                              " 	where a.PONo = '" & cboPartNo.Text & "' and a.AffiliateID = '" & Session("AffiliateID") & "' and a.SupplierID = '" & Session("SupplierID") & "' " & vbCrLf & _
                              " 	union all " & vbCrLf & _
                              " 	select  " & vbCrLf & _
                              " 		'' NoUrut, 'BY SUPPLIER' AffiliateName, '3' NoUrutDesc,  " & vbCrLf & _
                              " 		'' PartNo, b.PartNo PartNo1, '' PartName, ''KanbanCls, '' UnitDesc, 0 MOQ, 0 QtyBox, c.Maker, b.POQty, b.POQtyOld, e.Description CurrDesc, b.Price, b.Amount, " & vbCrLf & _
                              " 		ForecastN1 = isnull((select qty from MS_Forecast MF where MF.PartNo = b.PartNo and b.AffiliateID = MF.AffiliateID and YEAR(Period) = Year(DATEADD(MONTH,1,'" & Format(dtPeriodFrom.Value, "yyyy-MM-dd") & "')) and MONTH(Period) = MONTH(DATEADD(MONTH,1,'" & Format(dtPeriodFrom.Value, "yyyy-MM-dd") & "'))),0),  "

            ls_SQL = ls_SQL + " 		ForecastN2 = isnull((select qty from MS_Forecast MF where MF.PartNo = b.PartNo and b.AffiliateID = MF.AffiliateID and YEAR(Period) = Year(DATEADD(MONTH,2,'" & Format(dtPeriodFrom.Value, "yyyy-MM-dd") & "')) and MONTH(Period) = MONTH(DATEADD(MONTH,2,'" & Format(dtPeriodFrom.Value, "yyyy-MM-dd") & "'))),0),  " & vbCrLf & _
                              " 		ForecastN3 = isnull((select qty from MS_Forecast MF where MF.PartNo = b.PartNo and b.AffiliateID = MF.AffiliateID and YEAR(Period) = Year(DATEADD(MONTH,3,'" & Format(dtPeriodFrom.Value, "yyyy-MM-dd") & "')) and MONTH(Period) = MONTH(DATEADD(MONTH,3,'" & Format(dtPeriodFrom.Value, "yyyy-MM-dd") & "'))),0),  " & vbCrLf & _
                              "  		b.DeliveryD1, b.DeliveryD2, b.DeliveryD3, b.DeliveryD4, b.DeliveryD5, " & vbCrLf & _
                              "  		b.DeliveryD6, b.DeliveryD7, b.DeliveryD8, b.DeliveryD9, b.DeliveryD10,  " & vbCrLf & _
                              "  		b.DeliveryD11, b.DeliveryD12, b.DeliveryD13, b.DeliveryD14, b.DeliveryD15,  " & vbCrLf & _
            ...
            ls_SQL = ls_SQL + "  		b.DeliveryD11Old, b.DeliveryD12Old, b.DeliveryD13Old, b.DeliveryD14Old, b.DeliveryD15Old,  " & vbCrLf & _
                              ...
                              " 	inner join MS_UnitCls d on c.UnitCls = d.UnitCls " & vbCrLf & _
                              " 	left join MS_CurrCls e on e.CurrCls = b.CurrCls " & vbCrLf & _
                              " 	where a.PONo = '" & cboPartNo.Text & "' and a.AffiliateID = '" & Session("AffiliateID") & "' and a.SupplierID = '" & Session("SupplierID") & "' "
    ...
    Private Sub bindHeader(ByVal pPONO As String)
        ...
        Using sqlConn As New SqlConnection(clsGlobal.ConnectionString)
            ...
            ls_SQL = " select  " & vbCrLf & _
                  ...
                  " from PO_Master a " & vbCrLf & _
                  " left join PO_MasterUpload b on a.PONo = b.PONo and a.AffiliateID = b.AffiliateID and a.SupplierID = b.SupplierID" & vbCrLf & _
                  " where a.PONo = '" & pPONO & "' and a.AffiliateID = '" & Session("AffiliateID") & "' and a.SupplierID = '" & Session("SupplierID") & "' "
            ...
            If ds.Tables(0).Rows.Count > 0 Then
                ...
                txtPOEmergency.Text = clsPONo.POKanban(pPONO, Session("AffiliateID"), Session("SupplierID"))
                ...
                ButtonPartNo.JSProperties("cpPOKanban") = txtPOEmergency.Text
            Else
    ...
    Private Sub uf_Approve(ByVal pKanbanCls As String)
        ...
        Using sqlConn As New SqlConnection(clsGlobal.ConnectionString)
            ...
            Using sqlTran As SqlTransaction = sqlConn.BeginTransaction("AffiliateMaster")
                ls_sql = " Update PO_Master set FinalApproveDate = getdate(), FinalApproveUser = '" & Session("UserID") & "'" & vbCrLf & _
                            " WHERE AffiliateID = '" & Session("AffiliateID") & "' and PONo = '" & cboPartNo.Text & "' and SupplierID = '" & Session("SupplierID") & "'" & vbCrLf
                ...
                For j = 0 To grid.VisibleRowCount - 1
                    If grid.GetRowValues(j, "AffiliateName").ToString.Trim = "BY SUPPLIER" Then
                        ls_sql = " Update RemainingCapacity set QtyRemaining = QtyRemaining - " & grid.GetRowValues(j, "POQty").ToString.Trim & " " & vbCrLf & _
                            " WHERE Period = '" & Format(dtPeriodFrom.Value, "yyyyMM") & "' and SupplierID = '" & Session("SupplierID") & "' and PartNo = '" & grid.GetRowValues(j, "PartNo1").ToString.Trim & "'" & vbCrLf
                        sqlCommNew.CommandText = ls_sql
                ...
                'ls_sql = " select * from Kanban_Master where AffiliateID = '" & Session("AffiliateID") & "' and KanbanNo = '" & ls_KanbanNo & "-1" & "' and SupplierID = '" & ds.Tables(0).Rows(0)("SupplierID") & "'"
                ls_sql = " select max(convert(numeric,SUBSTRING(KanbanNo,10,2))) + 1 as SeqNo, AffiliateID  from Kanban_Master " & vbCrLf & _
                          " where AffiliateID = '" & Session("AffiliateID") & "' and SupplierID = '" & Session("SupplierID") & "' and YEAR(KanbanDate) = " & Year(dtPeriodFrom.Value) & " and month(KanbanDate) = " & Month(dtPeriodFrom.Value) & " " & vbCrLf & _
                ...
                If ds7.Tables(0).Rows.Count = 0 Then
                    ls_SeqNo = 1
                Else
                    ls_SeqNo = ds7.Tables(0).Rows(0)("SeqNo")
                End If
                ...
                For k = 1 To jlhHari
                    ...
                    ls_sql = " select  " & vbCrLf & _
                                  ...
                                  " 	e.DeliveryLocationCode, b.PartNo, d.UnitCls, CASE WHEN isnull(d.QtyBox,0) = 0 then d.MOQ else d.QtyBox end QtyBox, " & vbCrLf & _
                                  " 	[DeliveryD" & k & "], " & vbCrLf & _
                                  " 	colcycle1 =  CASE WHEN (CASE WHEN (DeliveryD" & k & " - CEILING(FLOOR(DeliveryD" & k & "/4) / CASE WHEN isnull(d.QtyBox,0) = 0 then d.MOQ else d.QtyBox end) * isnull(d.QtyBox,0)) > 0  " & vbCrLf & _
                                  "                          THEN CEILING(FLOOR(DeliveryD" & k & "/4) / CASE WHEN isnull(d.QtyBox,0) = 0 then d.MOQ else d.QtyBox end) * isnull(d.QtyBox,0)  " & vbCrLf & _
                                  "                          ELSE 0 END) = 0 THEN DeliveryD" & k & " ELSE  " & vbCrLf & _
                                  "                          (CASE WHEN (DeliveryD" & k & " - CEILING(FLOOR(DeliveryD" & k & "/4) / CASE WHEN isnull(d.QtyBox,0) = 0 then d.MOQ else d.QtyBox end) * isnull(d.QtyBox,0)) > 0  " & vbCrLf & _
                                  "                          THEN CEILING(FLOOR(DeliveryD" & k & "/4) / CASE WHEN isnull(d.QtyBox,0) = 0 then d.MOQ else d.QtyBox end) * isnull(d.QtyBox,0)  " & vbCrLf & _
                                  "                          ELSE 0 END) END ,  " & vbCrLf & _
                                  "  	colcycle2 =  CASE WHEN (DeliveryD" & k & " - CEILING(FLOOR(DeliveryD" & k & "/4) / CASE WHEN isnull(d.QtyBox,0) = 0 then d.MOQ else d.QtyBox end) * isnull(d.QtyBox,0)) > 0  " & vbCrLf & _
                                  "                          THEN CEILING(FLOOR(DeliveryD" & k & "/4) / CASE WHEN isnull(d.QtyBox,0) = 0 then d.MOQ else d.QtyBox end) * isnull(d.QtyBox,0)  " & vbCrLf & _
                                  "                          ELSE 0 END ,  " & vbCrLf & _
                                  "  	colcycle3 = CASE WHEN (DeliveryD" & k & " - CEILING(FLOOR(DeliveryD" & k & "/4) / CASE WHEN isnull(d.QtyBox,0) = 0 then d.MOQ else d.QtyBox end) * isnull(d.QtyBox,0)) > 0  " & vbCrLf & _
                                  "                          THEN CEILING(FLOOR(DeliveryD" & k & "/4) / CASE WHEN isnull(d.QtyBox,0) = 0 then d.MOQ else d.QtyBox end) * isnull(d.QtyBox,0)  "

                    ls_sql = ls_sql + "                          ELSE 0 END ,  " & vbCrLf & _
                                      "  	colcycle4 = CASE WHEN (DeliveryD" & k & " - (CEILING(FLOOR(DeliveryD" & k & "/4) / CASE WHEN isnull(d.QtyBox,0) = 0 then d.MOQ else d.QtyBox end) * isnull(d.QtyBox,0)) * 3) > 0  " & vbCrLf & _
                                      "                          THEN DeliveryD" & k & " - ((CEILING(FLOOR(DeliveryD" & k & "/4) / CASE WHEN isnull(d.QtyBox,0) = 0 then d.MOQ else d.QtyBox end) * isnull(d.QtyBox,0)) )*3  " & vbCrLf & _
                                      "                          ELSE 0 END " & vbCrLf & _
                                      " from PO_MasterUpload a " & vbCrLf & _
                                      ...
                                      " where a.AffiliateID = '" & Session("AffiliateID") & "' and a.PONo = '" & cboPartNo.Text & "' and a.SupplierID = '" & Session("SupplierID") & "' and b.KanbanCls = 0 and DeliveryD" & k & " > 0 "
                    ...
                    If ds.Tables(0).Rows.Count > 0 Then
                        ...
                        For i = 0 To ds.Tables(0).Rows.Count - 1
                            ...
                            Dim ls_Cycle As Integer = ds.Tables(0).Rows(i)("DeliveryD" & k) 'ds.Tables(0).Rows(i)("colcycle" & (j + 1)) [DeliveryD" & k & "
                            ...
                            For m = 0 To ls_Ulang - 1
                                Dim ls_Barcode As String = ls_KanbanNo.Trim & ls_SeqNo & "" & ds.Tables(0).Rows(i)("SupplierID").ToString.Trim & (m + 1) & Session("AffiliateID").ToString.Trim & ds.Tables(0).Rows(i)("PartNo").ToString.Trim
    ...
    Private Sub up_FillCombo(ByVal pPeriod As String)
        ...
        'ls_SQL = "select RTRIM(PONo) PONo from PO_MASTER where AffiliateID = '" & Session("AffiliateID") & "' and Year(Period) = '" & Year(dtPeriodFrom.Value) & "' and month(Period) = '" & Month(dtPeriodFrom.Value) & "' order by PONo " & vbCrLf
        ls_SQL = "select RTRIM(PONo) PONo from PO_MASTER where AffiliateID = '" & Session("AffiliateID") & "' and Year(Period) = '" & Year(pPeriod) & "' and month(Period) = '" & Month(pPeriod) & "' order by PONo " & vbCrLf
        Using sqlConn As New SqlConnection(clsGlobal.ConnectionString)
            sqlConn.Open()
    ...
    Private Sub sendEmail()
        ...
        ls_Body = clsNotification.GetNotification("24", "")
        ...
        mailMessage.Subject = "Final Approval PO No:" & cboPartNo.Text
