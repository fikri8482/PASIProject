...
Public Class SummaryInvoiceAffiliate
...
#Region "Declaration"
    ...
    Const colNet As Byte = 12
    Const colGross As Byte = 13
    Const colSupplierDelDate As Byte = 14
    Const colSupplierSJNo As Byte = 15
    Const colSupplierDeliveryQty As Byte = 16
    Const colPASIRecDate As Byte = 17
    Const colPASIReceivingQty As Byte = 18
    Const colInvoiceNoFromSupplier As Byte = 19
    Const colInvoiceDateFromSupplier As Byte = 20
    Const colInvoiceFromSupplierCurr As Byte = 21
    Const colInvoiceFromSupplierAmount As Byte = 22
    Const colPASIDelDate As Byte = 23
    Const colPASISJNo As Byte = 24
    Const colPASIDeliveryQty As Byte = 25
    Const colAffiliateRecDate As Byte = 26
    Const colAffiliateReceivingQty As Byte = 27
    Const colInvoiceNoToAffiliate As Byte = 28
    Const colInvoiceDateToAffiliate As Byte = 29
    Const colInvoiceToAffiliateCurr As Byte = 30
    Const colInvoiceToAffiliateAmount As Byte = 31
    Const colCount As Byte = 32
...
#Region "Procedures"
    ...
    Private Sub up_GridLoad()
        Using sqlConn As New SqlConnection(clsGlobal.ConnectionString)
            ...
            '         "    	POM.Period " & vbCrLf & _
            '         "    	,PONo = RTRIM(POM.PONo) " & vbCrLf & _
            '         " 	    ,KanbanNo = RTRIM(KM.KanbanNo) " & vbCrLf & _
            '         "    	,AffiliateID = RTRIM(POM.AffiliateID) " & vbCrLf & _
            '         "    	,SupplierID = RTRIM(POM.SupplierID) " & vbCrLf & _
            ...
            '         "    	,MPM.GrossWeight" & vbCrLf

            'ls_SQL = ls_SQL + "    	,ETDSupp = ES.ETDSupplier    " & vbCrLf & _
            '                  "    	,ETAAff = KM.KanbanDate    " & vbCrLf & _
            '                  "    	,PASIDeliveryDate = DOM.DeliveryDate    " & vbCrLf & _
            '                  "    	,PASISuratJalanNo = RTRIM(DOM.SuratJalanNo) " & vbCrLf & _
            '                  "  	,PASIDeliveryQty = DOD.DOQty  " & vbCrLf & _
            '                  "     ,AffiliateReceiveDate = RM.ReceiveDate  " & vbCrLf & _
            '                  "     ,AffiliateReceiveQty = ISNULL(RD.RecQty,0)  " & vbCrLf & _
            '                  " 	,InvoiceNo = RTRIM(PLM.InvoiceNo) " & vbCrLf & _
            '                  " 	,InvoiceDate = PLM.DeliveryDate " & vbCrLf & _
            '                  " 	,DOD.Price  " & vbCrLf & _
            '                  "  	,TOTAL = ISNULL(RD.RecQty,0) * ISNULL(DOD.Price,0)    " & vbCrLf
            ...
            '                  "  	INNER JOIN DOPASI_Detail DOD ON KD.AffiliateID = DOD.AffiliateID  " & vbCrLf & _
            ...
            '                  "  	INNER JOIN DOPASI_Master DOM ON DOM.AffiliateID = DOD.AffiliateID    " & vbCrLf & _
            '                  "    								AND DOM.SuratJalanNo = DOD.SuratJalanNo    " & vbCrLf & _
            '                  "    								AND DOM.SupplierID = DOD.SupplierID  " & vbCrLf & _
            '                  " 	LEFT JOIN PLPASI_Detail PLD ON DOD.SuratJalanNo = PLD.SuratJalanNo " & vbCrLf & _
            '                  " 								AND DOD.SupplierID = PLD.SupplierID " & vbCrLf

            'ls_SQL = ls_SQL + " 								AND DOD.AffiliateID = PLD.AffiliateID " & vbCrLf & _
            '                  " 								AND DOD.PONo = PLD.PONo " & vbCrLf & _
            '                  " 								AND DOD.PartNo = PLD.PartNo " & vbCrLf & _
            '                  " 	LEFT JOIN PLPASI_Master PLM ON PLD.SuratJalanNo = PLM.SuratJalanNo " & vbCrLf & _
            '                  " 								--AND PLD.SupplierID = PLM.SupplierID " & vbCrLf & _
            ...
            'ls_SQL = ls_SQL + "                                          AND RM.SuratJalanNo = RD.SuratJalanNo  " & vbCrLf & _
            '                  "                                          AND RD.PartNo = PLD.PartNo " & vbCrLf & _
            '                  "                                          AND RD.PONo = PLD.PONo " & vbCrLf & _
            ...
            '                  "  									AND DOM.DeliveryDate between MPR.Startdate and MPR.Enddate  " & vbCrLf & _
            '                  "  									AND MPR.AffiliateID = DOD.AffiliateID  " & vbCrLf & _
            ...
            'ls_SQL = ls_SQL + "   WHERE KD.KanbanQty > 0 " & vbCrLf
            ...
            ls_SQL = "sp_PASI_SummaryInvoiceAffiliate_GridLoad"
            ...
            'SUPPLIER DELIVERY DATE
            If chkSupplierDelDate.Checked = True Then
                cmd.Parameters.AddWithValue("@PASIDeliveryDateFrom", Format(dtSupplierDelDateFrom.Value, "yyyy-MM-dd"))
                cmd.Parameters.AddWithValue("@PASIDeliveryDateTo", Format(dtSupplierDelDateTo.Value, "yyyy-MM-dd"))
            End If
    ...
    Private Function GetSummaryOutStanding() As DataTable
        ...
        Try
            ...
            Using cn As New SqlConnection(clsGlobal.ConnectionString)
                ...
                'ls_sql = "   SELECT --DISTINCT " & vbCrLf &
                '     "    	POM.Period    " & vbCrLf &
                '     "    	,PONo = RTRIM(POM.PONo) " & vbCrLf &
                '     " 	    ,KanbanNo = RTRIM(KM.KanbanNo) " & vbCrLf &
                '     "    	,AffiliateID = RTRIM(POM.AffiliateID) " & vbCrLf &
                '     "    	,SupplierID = RTRIM(POM.SupplierID) " & vbCrLf &
                '     "    	,POKanban = CASE WHEN ISNULL(POD.KanbanCls, '0') = '1' THEN 'YES' ELSE 'NO' END    " & vbCrLf &
                '     "    	,POM.PASISendAffiliateDate    " & vbCrLf &
                '     "    	,PartNo = RTRIM(POD.PartNo) " & vbCrLf &
                '     "    	,PartName = RTRIM(MP.PartName) " & vbCrLf &
                '     "    	,QtyPO = ISNULL(POD.POQty,0)    " & vbCrLf &
                '     "    	,MPM.NetWeight" & vbCrLf &
                '     "    	,MPM.GrossWeight" & vbCrLf
                ...
                'SUPPLIER DELIVERY DATE
                If chkSupplierDelDate.Checked = True Then
                    cmd.Parameters.AddWithValue("@PASIDeliveryDateFrom", Format(dtSupplierDelDateFrom.Value, "yyyy-MM-01"))
                    cmd.Parameters.AddWithValue("@PASIDeliveryDateTo", Format(dtSupplierDelDateTo.Value, "yyyy-MM-" & ls_End))
                End If
    ...
    Private Sub epplusExportExcel(ByVal pFilename As String, ByVal pSheetName As String,
        ...
        Try
            Dim tempFile As String = "TemplateSummaryInvoiceAffiliate " & Format(Now, "yyyyMMdd hhmmss") & ".xlsx"
        ...
        Try
            ...
            Select Case pAction
                ...
                Case "excel"
                    ...
                    FileName = "TemplateSummaryInvoiceAffiliate.xlsx"
