...
Public Class clsDNDomestic
    Shared Sub up_DNDom(ByVal cfg As GlobalSetting.clsConfig,
        ...
        Dim cls As New clsDNDomesticProperty
...
step001:
            ...
            If ExcelSheet.Range("J27").Value Is Nothing Then
                ...
                Exit Try
            Else
                cls.SuratJalanNo = Microsoft.VisualBasic.Left(ExcelSheet.Range("J27").Value.ToString & "", 20)
            End If

            Try
                If ExcelSheet.Range("J25").Value Is Nothing = False Then cls.PIC = ExcelSheet.Range("J25").Value
                If ExcelSheet.Range("J29").Value Is Nothing = False Then cls.DriverName = ExcelSheet.Range("J29").Value
                If ExcelSheet.Range("J31").Value Is Nothing = False Then cls.DriverContact = ExcelSheet.Range("J31").Value
                If ExcelSheet.Range("J33").Value Is Nothing = False Then cls.NoPol = ExcelSheet.Range("J33").Value
                If ExcelSheet.Range("X27").Value Is Nothing = False Then cls.JenisArmada = ExcelSheet.Range("X27").Value
            Catch ex As Exception

            End Try

            'Check PO already Upload?
            If cekSJSubmited(cls.SuratJalanNo, cls.SupplierID, cls.AffiliateID, cfg.ConnectionString, errMsg) = True Then
                If Not IsNothing(ExcelBook) Then
                    ExcelBook.Save()
                    xlApp.Workbooks.Close()
                    xlApp.Quit()
                End If
                pub_ErrorMessage = "Can't Submit same Surat Jalan No. [" & cls.SuratJalanNo & "], Affiliate [" & cls.AffiliateID & "] becuase, already Receive in PASI system, Please check this file again!"
            ...
            Dim recExp As New List(Of clsDNDomesticProperty)
            Dim startRow As Integer = "39"
            ...
            For i = startRow To 10000
                ...
                Dim setNilai As New clsDNDomesticProperty
                ...
                setNilai.PIC = cls.PIC
                setNilai.DriverContact = cls.DriverContact
                setNilai.DriverName = cls.DriverName
                setNilai.NoPol = cls.NoPol
                setNilai.JenisArmada = cls.JenisArmada

                Try
                    If ExcelSheet.Range("D" & i).Value Is Nothing Then
                        If Not IsNothing(ExcelBook) Then
                            ExcelBook.Save()
                            xlApp.Workbooks.Close()
                            xlApp.Quit()
                        End If
                        pub_ErrorMessage = "ROW [" & i & "] PO No is blank, Please check this file again!"
                        sendEmailtoSupplier(GB, pAtttacment, pFileName, cls.SuratJalanNo, cls.PONo, cls.AffiliateID, cls.SupplierID, pub_ErrorMessage, errMsg)
                        ...
                        Exit Try
                    Else
                        setNilai.PONo = Trim(ExcelSheet.Range("D" & i).Value.ToString & "")
                    End If

                    If ExcelSheet.Range("L" & i).Value Is Nothing Then
                        If Not IsNothing(ExcelBook) Then
                            ExcelBook.Save()
                            xlApp.Workbooks.Close()
                            xlApp.Quit()
                        End If
                        pub_ErrorMessage = "ROW [" & i & "] Kanban No is blank, Please check this file again!"
                        sendEmailtoSupplier(GB, pAtttacment, pFileName, cls.SuratJalanNo, cls.PONo, cls.AffiliateID, cls.SupplierID, pub_ErrorMessage, errMsg)
                        ...
                        Exit Try
                    Else
                        setNilai.KanabanNo = Trim(ExcelSheet.Range("L" & i).Value.ToString & "")
                    End If

                    If ExcelSheet.Range("P" & i).Value Is Nothing Then
                        If Not IsNothing(ExcelBook) Then
                            ExcelBook.Save()
                            xlApp.Workbooks.Close()
                            xlApp.Quit()
                        End If
                        pub_ErrorMessage = "ROW [" & i & "] PartNo is blank, Please check this file again!"
                        sendEmailtoSupplier(GB, pAtttacment, pFileName, cls.SuratJalanNo, cls.PONo, cls.AffiliateID, cls.SupplierID, pub_ErrorMessage, errMsg)
                        ...
                        Exit Try
                    Else
                        setNilai.PartNo = Trim(ExcelSheet.Range("P" & i).Value.ToString & "")
                    End If

                    If cekPartNoAndPONo(setNilai.KanabanNo, setNilai.SupplierID, setNilai.AffiliateID, setNilai.PartNo, cfg.ConnectionString, setNilai.KanbanQty, setNilai.KanbanCls, errMsg) = False Then
                        If Not IsNothing(ExcelBook) Then
                            ExcelBook.Save()
                            xlApp.Workbooks.Close()
                            xlApp.Quit()
                        End If
                        pub_ErrorMessage = "ROW [" & i & "], Kanban No [" & setNilai.KanabanNo & "] with PartNo [" & setNilai.PartNo & "] not found with PASI System, Please check this file again!"
                        sendEmailtoSupplier(GB, pAtttacment, pFileName, cls.SuratJalanNo, cls.PONo, cls.AffiliateID, cls.SupplierID, pub_ErrorMessage, errMsg)
                        ...
                        Exit Try
                    End If
                    'setNilai.KanbanQty = KanbanQty
                    setNilai.QtyBox = clsGeneral.getQtyBox(GB, setNilai.PartNo, setNilai.SupplierID, setNilai.AffiliateID)
                    setNilai.ReceiveQty = clsGeneral.getReceiveQty(GB, setNilai.PartNo, setNilai.SupplierID, setNilai.AffiliateID, setNilai.KanabanNo)

                    Try
                        setNilai.DeliveryQty = Trim(ExcelSheet.Range("AO" & i).Value.ToString & "")
                    Catch ex As Exception
                        setNilai.DeliveryQty = 0
                    End Try

                    If setNilai.DeliveryQty Mod setNilai.QtyBox <> 0 Then
                        If Not IsNothing(ExcelBook) Then
                            ExcelBook.Save()
                            xlApp.Workbooks.Close()
                            xlApp.Quit()
                    ...
                    If (setNilai.DeliveryQty + setNilai.ReceiveQty) > setNilai.KanbanQty Then
                        If Not IsNothing(ExcelBook) Then
                            ExcelBook.Save()
                            xlApp.Workbooks.Close()
                            xlApp.Quit()
                        End If
                        pub_ErrorMessage = "ROW [" & i & "] DO Qty + already Receive Qty is bigger than Kanban Qty, Please check this file again!"
                        sendEmailtoSupplier(GB, pAtttacment, pFileName, cls.SuratJalanNo, cls.PONo, cls.AffiliateID, cls.SupplierID, pub_ErrorMessage, errMsg)
                ...
                If setNilai.DeliveryQty > 0 Then
                    recExp.Add(setNilai)
                End If
            Next
    ...
    Public Shared Function cekSJSubmited(ByVal SuratJalanNo As String, _
                                           ByVal SupplierID As String, ByVal AffiliateID As String, _
                                           ByVal pConstr As String, ByRef errMsg As String) As Boolean
        Dim sql As String = ""

        sql = "select * from ReceivePASI_Master WHERE SuratJalanNo = '" & Trim(SuratJalanNo) & "' and SupplierID = '" & Trim(SupplierID) & "' AND AffiliateID = '" & Trim(AffiliateID) & "' " 
        Try
            Using Cn As New SqlConnection(pConstr)
                Cn.Open()
                Dim cmd As New SqlCommand(sql, Cn)
    ...
    Public Shared Function cekPartNoAndPONo(ByVal KanbanNo As String, ByVal SupplierID As String, ByVal AffiliateID As String, ByVal PartNo As String, ByVal pConstr As String, ByRef KanbanQty As Double, ByRef KanbanCls As String, ByRef errMsg As String) As Boolean
        Dim sql As String = ""

        sql = " select a.KanbanQty, CASE WHEN b.KanbanStatus = 0 then 1 else 0 end KanbanCls from Kanban_Detail a left join Kanban_Master b on a.AffiliateID = b.AffiliateID and a.SupplierID = b.SupplierID and a.KanbanNo = b.KanbanNo" & vbCrLf & _
                " where a.KanbanNo = '" & KanbanNo & "' and a.AffiliateID = '" & AffiliateID & "' " & vbCrLf & _
                "       and a.SupplierID = '" & SupplierID & "' and a.PartNo = '" & PartNo & "' " & vbCrLf
        ...
        Try
            Using Cn As New SqlConnection(pConstr)
                ...
                If dt.Tables(0).Rows.Count > 0 Then
                    ...
                    KanbanQty = dt.Tables(0).Rows(0)("KanbanQty")
                    KanbanCls = dt.Tables(0).Rows(0)("KanbanCls")
                Else
    ...
    Shared Sub insertDetail(ByVal pMaster As clsDNDomesticProperty, ByVal pConstr As String, ByRef errMsg As String)
        Dim sql As String = ""
        Dim i As Integer
        ...
        Try
            Using cn As New SqlConnection(pConstr)
                ...
                sql = " UPDATE dbo.DOSUpplier_Detail " & vbCrLf & _
                        " SET DOQty = " & Trim(pMaster.DeliveryQty) & " " & vbCrLf & _
                        ...
                        " AND KanbanNo = '" & Trim(pMaster.KanabanNo) & "' AND PartNo = '" & Trim(pMaster.PartNo) & "' " & vbCrLf & _
                        " AND PONo = '" & Trim(pMaster.PONo) & "'"
                ...
                If i = 0 Then
                    sql = " INSERT INTO dbo.DOSUpplier_Detail " & vbCrLf & _
                            " (SuratJalanNo, SupplierID, AffiliateID, PONo, POKanbanCls, KanbanNo, PartNo, UnitCls, DOQty) " & vbCrLf & _
                            " VALUES  ( '" & Trim(pMaster.SuratJalanNo) & "' , " & vbCrLf & _
                            "           '" & Trim(pMaster.SupplierID) & "' , " & vbCrLf & _
                            "           '" & Trim(pMaster.AffiliateID) & "' ,  " & vbCrLf & _
                            "           '" & Trim(pMaster.PONo) & "' ,  " & vbCrLf & _
                            "           '" & Trim(pMaster.KanbanCls) & "' ,  " & vbCrLf & _
                            "           '" & Trim(pMaster.KanabanNo) & "' , " & vbCrLf & _
                            "           '" & Trim(pMaster.PartNo) & "' ,  " & vbCrLf & _
                            "           '01' , " & vbCrLf & _
                            "           '" & Trim(pMaster.DeliveryQty) & "'" & vbCrLf & _
                            "       ) "
                    cmd.CommandText = sql
                    i = cmd.ExecuteNonQuery()
                End If
