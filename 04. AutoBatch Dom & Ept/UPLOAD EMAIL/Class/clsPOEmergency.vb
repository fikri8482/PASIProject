...
Public Class clsPOEmergency
    Shared Sub up_POEmergency(ByVal cfg As GlobalSetting.clsConfig,
...
step001:
            ...
            If ExcelSheet.Range("P9").Value Is Nothing Then
                tempOrderNo = Microsoft.VisualBasic.Left(ExcelSheet.Range("I9").Value.ToString & "", 20)
            Else
                tempOrderNo = Microsoft.VisualBasic.Left(ExcelSheet.Range("P9").Value.ToString & "", 20)
            End If
            ...
            Try
                cls.ETDVendor = clsGeneral.getEmergencyETD(GB, cls.PONo, cls.OrderNo, cls.SupplierID, cls.AffiliateID)
            ...
            Dim recExp As New List(Of clsPOEmergencyProperty)
            ...
            For i = startRow To 10000
                ...
                Dim setNilai As New clsPOEmergencyProperty
                ...
                'Check Approve Qty <> 0
                Try
                    setNilai.ApproveQty = Trim(ExcelSheet.Range("AE" & i).Value.ToString & "")
                Catch ex As Exception
                    If Not IsNothing(ExcelBook) Then
                        ExcelBook.Save()
                        xlApp.Workbooks.Close()
                        xlApp.Quit()
                    End If
                    pub_ErrorMessage = "ROW [" & i & "] Approve Qty is blank, Please check this file again!"
                    sendEmailtoSupplier(GB, pAtttacment, pFileName, cls.PONo, cls.AffiliateID, cls.SupplierID, pub_ErrorMessage, errMsg)
                ...
                Try
                    setNilai.ApproveQtyOld = clsGeneral.getApproveQtyPO(GB, setNilai.PONo, setNilai.OrderNo, setNilai.SupplierID, setNilai.AffiliateID, setNilai.PartNo)
                Catch ex As Exception
                    If Not IsNothing(ExcelBook) Then
                        ExcelBook.Save()
                        xlApp.Workbooks.Close()
                ...
                Try
                    If setNilai.ApproveQty Mod setNilai.QtyBox <> 0 Then
                        If Not IsNothing(ExcelBook) Then
                            ExcelBook.Save()
                            xlApp.Workbooks.Close()
                            xlApp.Quit()
